...
{
	CString strURI;
	strURI.Format(_T("XGMESLogic\\AODMag\\CAOD_Prod_Area\\S25A01"),GetIdentity().ToCString());
...
CString CAOD_Unit_Mag::GetPrepareArea()
{
	CString strID = GetIdentity().ToCString();
	CString strPrepare = strID + _T("A01");
	CString strAreaPrepare = CL3NameParser::MergeClassDomainName(_T("XGMESLogic\\AODMag\\CAOD_Prod_Area"),strPrepare);
	return strAreaPrepare;
}

//CString CAOD_Unit_Mag::GetPrepareArea()//2017-09-05 修改
//{
//	CString strID = GetIdentity().ToCString();
//	CString strPrepare = strID + _T("P01");
//	CString strAreaPrepare = CL3NameParser::MergeClassDomainName(_T("XGMESLogic\\AODMag\\CAOD_Prod_Area"),strPrepare);
//	return strAreaPrepare;
//}
...
CString CAOD_Unit_Mag::FindNextHeatIDFromPlan()
{
	...
	else if(csUnitID == "S51")
	{
		csSQL.Format(_T("select min(HeatID) from CPlan_Tapping where Status ='%s' and BOFID ='%s' "),TAPPING_PLAN_STATUS_EXCUTING,csUnitID);
	}
...
L3LONG CAOD_Unit_Mag::DesignateMainMaterial(L3RECORDSET rsIrons,L3RECORDSET rsReturnSteel,L3RECORDSET rsScrapes)
{
	...
	rsIronsDesignated.Attach(GetInputSteels());//2017-09-05
...
L3LONG CAOD_Unit_Mag::AutoDesignateMainMaterial()
{
	...
	//CString csSQL1;
	//CL3RecordSetWrap rsDepFeed1;
	//csSQL.Format(_T("select * from cmelt_feed_data where HeatID ='%s'"),HeatID);
	//if(!Query(csSQL1,rsDepFeed1) || rsDepFeed1.IsNull() || rsDepFeed1.GetRowCount() <1)
	//	return -13;
	//rsDepFeed1.MoveFirst();
 //  /////////
 //  /* if(rsDepFeed.GetRowCount() <1 || rsDepFeed.IsNull() )
	//{
	//	rsFeedData.CopyFrom(rsDepFeed1);
	//}
	//else if (rsDepFeed1.GetRowCount() <1 || rsDepFeed1.IsNull() )
	//{
...
//////////////////end 20171012
	...
	CString csSQL1;
	CL3RecordSetWrap rsDepFeed1;
	csSQL1.Format(_T("select * from cmelt_feed_data where HeatID ='%s'"),HeatID);
	rsDepFeed1.MoveFirst();

	if(!Query(csSQL,rsDepFeed) || rsDepFeed.IsNull() || rsDepFeed.GetRowCount() <1)
	{
		rsFeedData.CopyFrom(rsDepFeed1);
		//rsFeedData.SetFieldByName(_T("IronWeight"),rsDepFeed1.GetFieldByName(_T("Weight")).ToDouble());
	}
	else if(!Query(csSQL1,rsDepFeed1) || rsDepFeed1.IsNull() || rsDepFeed1.GetRowCount() <1)
	{
		rsFeedData.CopyFrom(rsDepFeed);
		//rsFeedData.SetFieldByName(_T("IronWeight"),rsDepFeed.GetFieldByName(_T("Weight")).ToDouble());
	}
	...
	rsDepFeed.MoveFirst();
	rsDepFeed1.MoveFirst();
	if (rsDepFeed.GetFieldByName(_T("Weight")).ToLong()  != 0 || rsDepFeed.GetFieldByName(_T("Weight")).ToLong()  != NULL  )
	{
		rsFeedData.SetFieldByName(_T("IronWeight"),rsDepFeed.GetFieldByName(_T("Weight")).ToDouble());
	}
	else if (rsDepFeed1.GetFieldByName(_T("Weight")).ToLong()  != 0 || rsDepFeed1.GetFieldByName(_T("Weight")).ToLong()  != NULL )
	{
		rsFeedData.SetFieldByName(_T("IronWeight"),rsDepFeed1.GetFieldByName(_T("Weight")).ToDouble());
	}
	//100T天车名称2016-05-06
...
///////////////////////////////添加脱磷发送铁水温度
	...
	CString csSQLTemp1;
	CL3RecordSetWrap rsDepTemp1;
	csSQLTemp1.Format(_T("select * from cmelt_process_data where HeatID ='%s'"),HeatID);
	/*if(!Query(csSQLTemp1,rsDepTemp) || rsDepTemp1.IsNull() || rsDepTemp1.GetRowCount() <1)
		return -19;*/
	rsDepTemp1.MoveFirst();
    /*L3LONG longTemp;
	L3LONG longTemp1;
	longTemp = rsDepTemp.GetFieldByName(_T("Tapping_Temp")).ToLong();
	longTemp1 = rsDepTemp1.GetFieldByName(_T("Tapping_Temp")).ToLong();*/
	if(!Query(csSQLTemp,rsDepTemp) || rsDepTemp.IsNull() || rsDepTemp.GetRowCount() <1)
	{
		rsFeedData.SetFieldByName(_T("IronTemp"),rsDepTemp1.GetFieldByName(_T("Tapping_Temp")).ToDouble());
	}
	else if(!Query(csSQLTemp1,rsDepTemp1) || rsDepTemp1.IsNull() || rsDepTemp1.GetRowCount() <1)
	{
...
L3BOOL CAOD_Unit_Mag::CaculateLadleGrade(L3DATETIME dtTapTime,L3STRING strHeatID)
{
	...
	while(!rsLadle.IsEOF())
	{
		...
		else
		{
            ...
            SetObjectPropValue(strProcData,_T("Ladle_Age"),CL3Variant(L3LONG(nAge)));
...
L3RECORDSET CAOD_Unit_Mag::GetInputIrons()
{
	...
	//CString strIronType = MATERIAL_STEEL_URI;
...
L3RECORDSET CAOD_Unit_Mag::GetInputSteels()
{
	...
	//CString strCache = _T("XGMESLogic\\BaseDataMag\\XGInterCache\\S25A01");
...
/// <Method class="CAOD_Unit_Mag" name="DesignateSteel" type="L3BOOL">
/// 指定下个上线的钢水.
/// <Param name="rsSteel" type="L3RECORDSET">要指定上线的钢水记录</Param>
/// </Method>
L3BOOL CAOD_Unit_Mag::DesignateSteel(L3RECORDSET rsSteel)
{
...
/// 处理大包钢水到达事件，将指定的钢水上线。
/// </Method>
L3BOOL CAOD_Unit_Mag::SteelArrived()
{
	LONG nTrans = 0;
	if(!IsInTrans())
	{
...
//	NextHeatID = strHeatID;
	...
	CString strAreaURI = GetPrepareArea(); // 上线到AOD工位
	// 检查AOD工位是否存在大包
	...
	// 修改铸机状态2008-12-23
	/*if(!SetPropValue(_T("Status"),CL3Variant(L3LONG(CAOD_Unit_Mag::CCMArrivle))))
	{
		if(nTrans > 0)
			Rollback(nTrans);
		return FALSE;
