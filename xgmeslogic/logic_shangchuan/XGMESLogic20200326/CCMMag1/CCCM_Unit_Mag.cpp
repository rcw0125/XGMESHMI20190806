...
L3BOOL CCCM_Unit_Mag::StartCasting()
{
  ...
  //  COleDateTimeSpan span = COleDateTime::GetCurrentTime()-dtCurrentTime;
   ...
   //}
    ...
    //修正钢包基本数据里的烧眼信息//2017-04-05
	CString csSQL1 =  _T("select * from CLadle_Use where HeatID = '%s'");
	csSQL1.Format(csSQL1,strHeatID);
	CL3RecordSetWrap rsLadle;
	if(!Query(csSQL1,rsLadle))
	{
		if(nTrans > 0) Rollback(nTrans);
		return FALSE;
	}
	...
	CString strLadleUse = CL3NameParser::MergeClassDomainName(CLADLE_USE_URI,csGuid);
	SetObjectPropValue(strLadleUse,_T("Auto_Flow"),CL3Variant(L3LONG(1)));
	SetObjectPropValue(strLadleUse,_T("Irr_Start_Time"),CL3Variant(curTime));
...
L3BOOL CCCM_Unit_Mag::CompletCasting()
{
	...
	//修正钢包基本数据里的本炉次停浇时间//2017-04-01
	CString csSQL =  _T("select * from CLadle_Use where HeatID = '%s'");
	...
	SetObjectPropValue(strLadleUse,_T("Casting_Stop_Time"),CL3Variant(curTime));
	COleDateTime tapTime =  (rsLadle.GetFieldByName(_T("TAP_TIME"))).ToDateTime(); //过钢时间
	
    COleDateTimeSpan spanAfterSteel = curTime - tapTime;//过钢时间
	L3LONG timeAfterSteel = (LONG)spanAfterSteel.GetTotalMinutes();//过钢时间
    SetObjectPropValue(strLadleUse,_T("Time_After_Steel"),CL3Variant(timeAfterSteel));
...
L3BOOL CCCM_Unit_Mag::ChangeCastingNo()
{
	...
	// 取得当前浇次
	//GetObjectPropValue(CCM_CLASS_URI + _T("\\") + GetURI(),_T("CastingNo"))).ToCString()
	//CString strCurNo = CastingNo;  20190115修改 浇次从数据库读取

	// 查询当前浇次号
	CString strccmCurNo; strccmCurNo.Format(_T("select CastingNo from cccm_unit_mag where name = '%s' "), (LPCTSTR)GetIdentity().ToCString());
	CL3RecordSetWrap rs;
	if(!Query(strccmCurNo,rs) || rs.GetRowCount() < 1)
	{
		if(nTrans > 0) Rollback(nTrans);
		return FALSE;
	}
	rs.MoveFirst();
	//CString strCurNo = (GetObjectPropValue(CCM_CLASS_URI + _T("\\") + GetIdentity().ToCString(),_T("CastingNo"))).ToCString();
	CString strCurNo = rs.GetFieldByName(_T("CastingNo")).ToCString();
	strCurNo.Trim();
...
L3BOOL CCCM_Unit_Mag::ChangeTundishNo()
{
	...
	// 取得当前包次
	//(GetObjectPropValue(CCM_CLASS_URI + _T("\\") + GetIdentity().ToCString(),_T("CastingNo"))).ToCString();
	//CString strCurNo = TundishNo;
    // 查询当前包次号
	CString strccmCurTundishNo; strccmCurTundishNo.Format(_T("select TundishNo from cccm_unit_mag where name = '%s' "), (LPCTSTR)GetIdentity().ToCString());
	CL3RecordSetWrap rsTundishNo;
	if(!Query(strccmCurTundishNo,rsTundishNo) || rsTundishNo.GetRowCount() < 1)
	{
		if(nTrans > 0) Rollback(nTrans);
		return FALSE;
	}
	rsTundishNo.MoveFirst(); 
	CString strCurNo = rsTundishNo.GetFieldByName(_T("TundishNo")).ToCString();
	strCurNo.Trim();
...
L3BOOL CCCM_Unit_Mag::AcceptCCMTundishFireData(L3RECORDSET rsData)
{
	...
	// 查询当前东中包,西中包  20190116 修改
	CString strccmCurNo; strccmCurNo.Format(_T("select EastTundishID,WestTundishID from cccm_unit_mag where name = '%s' "), (LPCTSTR)GetIdentity().ToCString());
	CL3RecordSetWrap rsccm;
	if(!Query(strccmCurNo,rsccm) || rsccm.GetRowCount() < 1)
	{	
		return FALSE;
	}
	rsccm.MoveFirst();
	...
	while (!rsTemp.IsEOF())
	{
		...
		if (csPosition == _T("1")) //东中包
		    ...
		    csTundishID = rsccm.GetFieldByName(_T("EastTundishID")).ToCString();
		else  //西中包
			//csTundishID = WestTundishID;
		    csTundishID = rsccm.GetFieldByName(_T("WestTundishID")).ToCString();
		...
		// 查询中包的工作层编号  20190116 修改
	    CString strTundish; strTundish.Format(_T("select WorkID from CTundish_Base where TundishID = '%s' "), (LPCTSTR)csTundishID);
	    CL3RecordSetWrap rsTundish;
		if(!Query(strTundish,rsTundish) || rsTundish.GetRowCount() < 1)
		{	
			return FALSE;
		}
		rsTundish.MoveFirst();

		rsTemp.SetFieldByName(_T("WorkID"),CL3Variant(rsTundish.GetFieldByName(_T("WorkID")).ToCString()));
...
L3BOOL CCCM_Unit_Mag::AcceptCCMRuntimeData(L3RECORDSET rsData)
{
	...
	// 查询当前炉号，处理号， 20190116 修改
	CString strccmCurNo; strccmCurNo.Format(_T("select TreatNo,HeatID,TundishWeight,OnOff1st,OnOff2st,OnOff3st,OnOff4st from cccm_unit_mag where name = '%s' "), (LPCTSTR)GetIdentity().ToCString());
	CL3RecordSetWrap rsccm;
	if(!Query(strccmCurNo,rsccm) || rsccm.GetRowCount() < 1)
	{	
		return FALSE;
	...
	while (!rsTemp.IsEOF())
	{
		...
		rsTemp.SetFieldByName(_T("TreatNo"),CL3Variant(rsccm.GetFieldByName(_T("TreatNo")).ToCString()));
		//20190116
		//rsTemp.SetFieldByName(_T("HeatID"),CL3Variant(HeatID));
		rsTemp.SetFieldByName(_T("HeatID"),CL3Variant(rsccm.GetFieldByName(_T("HeatID")).ToCString()));
		...
		if (CatchCount == 3)  //用中包当前钢水设置中包液面数据项 
		{
			...
			if ((cstrCCID != _T("S61")) && (cstrCCID != _T("S62")))//2009-04-30
			{
				...
				SetObjectPropValue(procData,_T("Tundish_Level_Mold"),CL3Variant(rsccm.GetFieldByName(_T("TundishWeight")).ToDouble()));  //中包液面
			}
				...
				if (strGUID.GetLength() > 1)
				{
   					...
   					switch (i)
					{
    					case CCCM_Unit_Mag::ST1:
							...
							SetObjectPropValue(strObjectID,_T("CastingStatus"),CL3Variant((LONG)rsccm.GetFieldByName(_T("OnOff1st")).ToLong()));
						break;
						case CCCM_Unit_Mag::ST2: 
							//20190116
							//SetObjectPropValue(strObjectID,_T("CastingStatus"),CL3Variant((LONG)OnOff2st));
							SetObjectPropValue(strObjectID,_T("CastingStatus"),CL3Variant((LONG)rsccm.GetFieldByName(_T("OnOff2st")).ToLong()));
						break;
						case CCCM_Unit_Mag::ST3: 
							//20190116
							//SetObjectPropValue(strObjectID,_T("CastingStatus"),CL3Variant((LONG)OnOff3st));
							SetObjectPropValue(strObjectID,_T("CastingStatus"),CL3Variant((LONG)rsccm.GetFieldByName(_T("OnOff3st")).ToLong()));
						break;
						case CCCM_Unit_Mag::ST4: 
							//20190116
							//SetObjectPropValue(strObjectID,_T("CastingStatus"),CL3Variant((LONG)OnOff4st));
							SetObjectPropValue(strObjectID,_T("CastingStatus"),CL3Variant((LONG)rsccm.GetFieldByName(_T("OnOff4st")).ToLong()));
						break;
						default:
							break;
					}
...
L3BOOL CCCM_Unit_Mag::AcceptCCMSpeedData(L3RECORDSET rsData)
{
	...
	// 查询当前炉号 20190116 修改
	CString strccmCurNo; strccmCurNo.Format(_T("select HeatID from cccm_unit_mag where name = '%s' "), (LPCTSTR)GetIdentity().ToCString());
	CL3RecordSetWrap rsccm;
	if(!Query(strccmCurNo,rsccm) || rsccm.GetRowCount() < 1)
	{	
		return FALSE;
	...
	while (!rs.IsEOF())
	{
		...
		if (fSpeed1ST > 0.001)
		{
			...
			rsSpeed.SetFieldByName(_T("HeatID"),CL3Variant(rsccm.GetFieldByName(_T("HeatID")).ToCString()));
...
L3BOOL CCCM_Unit_Mag::AcceptCCMTundishTempData(L3RECORDSET rsData)
{
	...
	{
		rsTemp.SetFieldByName(_T("CCMID"),CL3Variant(GetIdentity().ToCString()));
		//rsTemp.SetFieldByName(_T("HeatID"),CL3Variant(HeatID));  20190119 RCW xiugai
		rsTemp.SetFieldByName(_T("HeatID"),CL3Variant(rsccm.GetFieldByName(_T("HeatID")).ToCString()));	
...
L3BOOL CCCM_Unit_Mag::LadleDeparture()
{
   ...
   //修正本炉次钢包实绩中的离站数据
	...
	SetObjectPropValue(strLadleUse,_T("Off_Ladle_Time"),CL3Variant(curTime));*/
...
L3BOOL CCCM_Unit_Mag::AcceptCCMTundishTempForRefine(L3RECORDSET rsData)
{
    ...
    // 查询当前炉号，处理号， 20190116 修改
	CString strccmCurNo; strccmCurNo.Format(_T("select TreatNo,HeatID,CastingNo from cccm_unit_mag where name = '%s' "), (LPCTSTR)GetIdentity().ToCString());
	CL3RecordSetWrap rsccm;
	if(!Query(strccmCurNo,rsccm) || rsccm.GetRowCount() < 1)
	{	
		return FALSE;
	...
	while (!rsTemp.IsEOF())
	{
		...
		rsTemp.SetFieldByName(_T("HeatID"),CL3Variant(HeatID));
		rsTemp.SetFieldByName(_T("TreatNo"),CL3Variant(TreatNo));*/
		rsTemp.SetFieldByName(_T("CastingNo"),CL3Variant(rsccm.GetFieldByName(_T("CastingNo")).ToCString()));
		rsTemp.SetFieldByName(_T("HeatID"),CL3Variant(rsccm.GetFieldByName(_T("HeatID")).ToCString()));
		rsTemp.SetFieldByName(_T("TreatNo"),CL3Variant(rsccm.GetFieldByName(_T("TreatNo")).ToCString()));
		rsTemp.SetFieldByName(_T("Measure_Time"),CL3Variant(COleDateTime::GetCurrentTime()));	
		rsTemp.MoveNext();
...
L3BOOL CCCM_Unit_Mag::AcceptTundishWeightData(L3RECORDSET rsData)
{
	...
	// 查询当前处理号， 20190116 修改
	CString strccmCurNo; strccmCurNo.Format(_T("select TreatNo from cccm_unit_mag where name = '%s' "), (LPCTSTR)GetIdentity().ToCString());
	CL3RecordSetWrap rsccm;
	if(!Query(strccmCurNo,rsccm) || rsccm.GetRowCount() < 1)
	{	
		return FALSE;
    ...
    //CString csTreatNo =GetPropValue(_T("TreatNo")).ToCString();
	CString csTreatNo =rsccm.GetFieldByName(_T("TreatNo")).ToCString();
	CString csHeatLadleUri = CCM_HEAT_LADLE_CLASS_URI + _T("\\") + csTreatNo;
