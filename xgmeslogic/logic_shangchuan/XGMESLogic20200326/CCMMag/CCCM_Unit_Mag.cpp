...
L3BOOL CCCM_Unit_Mag::StartCasting()
{
  ...
  //  COleDateTimeSpan span = COleDateTime::GetCurrentTime()-dtCurrentTime;
   ...
   //}
    ...
    COleDateTime tapTime =  (rsLadle.GetFieldByName(_T("TAP_TIME"))).ToDateTime(); //出钢时间

	CString strLadleUse = CL3NameParser::MergeClassDomainName(CLADLE_USE_URI,csGuid);
	//添加开始 2019-03-13功能 增加引流沙过钢时间  就开浇时间减去出钢时间 2019-03-13
	
	COleDateTimeSpan spanAfterSteel = curTime - tapTime;//引流沙过钢时间=当前时间（开浇）-出钢时间
	LONG timeAfterSteel = (LONG)spanAfterSteel.GetTotalMinutes();//引流沙过钢时间
	SetObjectPropValue(strLadleUse,_T("Time_Ladle_Casting"),CL3Variant(timeAfterSteel));
	//添加结束 2019-03-13
...
L3BOOL CCCM_Unit_Mag::CompletCasting()
{
    ...
    COleDateTimeSpan spanAfterSteel = curTime - tapTime;//过钢时间=当前时间（停浇）-出钢时间
	//L3LONG timeAfterSteel = (LONG)spanAfterSteel.GetTotalMinutes();//过钢时间
	LONG timeAfterSteel = (LONG)spanAfterSteel.GetTotalMinutes();//过钢时间
