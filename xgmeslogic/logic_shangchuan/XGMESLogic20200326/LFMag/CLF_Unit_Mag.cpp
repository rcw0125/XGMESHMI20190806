...
/// <Param name="fWeight" type="L3DOUBLE">重量</Param>
/// </Method>
L3BOOL CLF_Unit_Mag::SetWeightNew(L3DOUBLE fWeight,L3DOUBLE tWeight,L3DOUBLE kzWeight)
{
	LONG nTrans = 0;
	if(!IsInTrans())
	{
	...
	double netWeight=fWeight-tWeight-kzWeight;
	...
	if(fOldWeight < 0.1)//
	{
		if(!SetObjectPropValue(strProcData,_T("Tapping_Weight"),CL3Variant(netWeight)))
		{
			if(nTrans > 0) Rollback(nTrans);
			return FALSE;
		}
		
		if(!SetObjectPropValue(strURI,_T("Weight"),CL3Variant(netWeight)))
		{
			if(nTrans > 0) Rollback(nTrans);
			return FALSE;
		}
	...
	rsLog.AppendRecord();
	rsLog.SetFieldByName(_T("LFID"),CL3Variant(strLFID));
	rsLog.SetFieldByName(_T("Weight"),CL3Variant(netWeight));
	rsLog.SetFieldByName(_T("GrossWeight"),CL3Variant(fWeight));
	rsLog.SetFieldByName(_T("TareWeight"),CL3Variant(tWeight));
	rsLog.SetFieldByName(_T("KzWeight"),CL3Variant(kzWeight));
	rsLog.SetFieldByName(_T("Status"),CL3Variant((LONG)1));
	rsLog.SetFieldByName(_T("LadleNo"),GetObjectPropValue(strURI,_T("LadleID")));
	rsLog.SetFieldByName(_T("Log_Time"),CL3Variant(COleDateTime::GetCurrentTime()));
	rsLog.SetFieldByName(_T("HeatID"),CL3Variant(csLadleHeatID));
	...
	if(rsLF.GetRowCount() > 0)
	{
		...
		while(!rsLF.IsEOF())
		{
			...
			if(cstrTreatNo.GetLength() > 0)
			{
				...
				if(fLFProWeight < 0.1)
				{
					SetObjectPropValue(strLFProcData,_T("Steel_Weight"),CL3Variant(netWeight));
				}
			}
			rsLF.MoveNext();
		}
...
L3BOOL CLF_Unit_Mag::StartProcess(L3STRING Area)
{
	...
	/*	lWallTemp = GetObjectPropValue(CLADLE_USE_URI,_T("Wall_Temp")).ToLong();*/
	}
	...
	/*rsLadleInfo.SetFieldByName(_T("Wall_Temp"),CL3Variant(lWallTemp));*/
	...
	{
		/**********************************************20090902 liao********************************************************************/
		csTemp =  _T("SELECT stop_time_len, note,wall_temp			");
		...
		if(!(rsResult.IsNull()) && rsResult.GetRowCount() > 0)
		{
			...
			rsLadleInfo.SetFieldByName(_T("Wall_Temp"),rsResult.GetFieldByName(_T("wall_temp")));
		}
		/*--------------------------------------------------------------------------------------------------------------*/
	...
	if(lLadleAge > 1 )
	{
		...
		//////////csTemp += _T("                   WHERE ladleno = '%s'    			");            20190603添加 cut_operator 是折包钢包号
		csTemp += _T("                   WHERE ladleno = '%s' or 	cut_operator = '%s'				");
		csTemp += _T("                ORDER BY productiondate DESC)									");
		csTemp += _T("         WHERE ROWNUM < 2) a,													");
		csTemp += _T("       cccm_process_data b													");
		csTemp += _T(" WHERE a.treatno = b.treatno													");

        ///csSQL.Format(csTemp,csLadleID,csLadleID);    20190603 
		csSQL.Format(csTemp,csLadleID,csLadleID);
		rsResult.DeleteAll();
		Query(csSQL,rsResult);
		if(!(rsResult.IsNull()) && rsResult.GetRowCount() > 0)
		{
...
L3BOOL CLF_Unit_Mag::StartHeating()
{
	...
	else
	{
		...
		rsStatus.DelField(_T("GUID"));
		rsStatus.AppendRecord();
		rsStatus.SetFieldByName(_T("TreatNo"),CL3Variant(NextTreatNo));
		LONG nStatus=2;
		rsStatus.SetFieldByName(_T("Status"),CL3Variant(nStatus));
		rsStatus.SetFieldByName(_T("Change_Time"),CL3Variant(COleDateTime::GetCurrentTime()));
		if(CreateNewObjects(LF_PROCESS_STATUS_CLASS_URI,rsStatus) < 1)
...
L3BOOL CLF_Unit_Mag::SteelLeave(L3STRING Area)
{
	...
	//////////////
	LONG nTrans = 0;
    CString csSQL =  _T("select * from CPlan_Tapping where HeatID = '%s'");
	csSQL.Format(csSQL,strHeatID);
	CL3RecordSetWrap rsPlanTapping;
	if(!Query(csSQL,rsPlanTapping))
	{
		if(nTrans > 0) Rollback(nTrans);
		return FALSE;
	}
	rsPlanTapping.MoveFirst();
	CString csType = rsPlanTapping.GetFieldByName(_T("Refine_Type")).ToCString(); 
	if(csType !="LR")
	{
		if(iAreaStatus <= CLF_Unit_Mag::LFBlowBegin )//2017-10-27修改 状态判定，过lf必须进行软吹。
		{
			SetL3LastError(XGMES_ERR_LF_WRONGSTATUSBLOW);
			return FALSE;
		}
		/*	if(iAreaStatus < CLF_Unit_Mag::LFProcEnd||iAreaStatus==CLF_Unit_Mag::LFBlowBegin)
		{
...
L3BOOL CLF_Unit_Mag::AcceptLFWeightData(L3RECORDSET rsData)
{
	...
	if (nWeightType == 1) //毛重
	{
		...
		if(csHeatID.GetLength() > 0)
		{
			...
			//通过炉号获取钢种 2018-06-07 yuan
			CString csUriNew = CL3NameParser::MergeClassDomainName(BOF_BASE_DATA_CLASS_URI,csHeatID);
			CString cstrSteelGrade = GetObjectPropValue(csUriNew,_T("SteelGrade")).ToCString();
			//CString cstrSteelGrade = _T("55SiCrA");
			cstrSteelGradeIndex = GetObjectPropValue(csUriNew,_T("SteelGradeIndex")).ToCString();
			//cstrSteelGradeIndex =  _T("J1701010");
			//通过炉号获取工艺路径 2018-06-07 yuan
			CString csUriRoute = CL3NameParser::MergeClassDomainName(PLAN_TAPPING_CLASS_URI,csHeatID);
			cstrRHID = GetObjectPropValue(csUriRoute,_T("RHID")).ToCString();
			//cstrRHID =  _T("S51");
			CString subStrSteelGrade1 = _T("CH1T");
			CString subStrSteelGrade2 = _T("CH1T-1");
			CString subStrSteelGrade3 = _T("DT4");
			CString subStrSteelGrade4 = _T("XG04XW");
			CString subStrSteelGrade5 = _T("XGYT0");
			CString subStrSteelGrade6 = _T("XGL1");
			CString subStrSteelGrade7 = _T("CH1A");
			CString subStrSteelGrade8 = _T("10B21-C");
			CString subStrSteelGrade9 = _T("10B21-A");
			CString subStrSteelGrade10 = _T("9Cr3Mo");
			CString subStrSteelGrade11 = _T("9Cr2Mo");
			CString subStrSteelGrade12 = _T("70Cr3Mo");
			CString subStrSteelGrade13 = _T("SWRH82B");
			CString subStrSteelGrade14 = _T("SWRH77B");
			CString subStrSteelGrade15 = _T("GCr15");
			CString subStrSteelGrade16 = _T("SWRS82B");
			CString subStrSteelGrade17 = _T("SWRS87B");
			CString subStrSteelGrade18 = _T("72A");
			CString subStrSteelGrade19 = _T("82A");
			CString subStrSteelGrade20 = _T("86A");
			CString subStrSteelGrade21 = _T("92A");
			CString subStrSteelGrade22 = _T("55SiCrA");
			CString subStrSteelGrade23 = _T("55SiCrA-1");
			CString subStrSteelGrade24 = _T("Q55SiCrA-1");
			CString subStrSteelGrade25 = _T("60Si2Mn");
			CString subStrSteelGrade26 = _T("SCM435-C");
			CString subStrSteelGrade27 = _T("SCM435-A");
			CString subStrSteelGrade28 = _T("SCM435");
			CString subStrSteelGrade29 = _T("XGM6");
			CString subStrSteelGrade30 = _T("XGYT1");
			CString subStrSteelGrade31 = _T("XGDL");
		
			idx1=cstrSteelGrade.Find(subStrSteelGrade1);//在cstrSteelGrade中查找subStrSteelGrade1.
			idx2=cstrSteelGrade.Find(subStrSteelGrade2);
			idx3=cstrSteelGrade.Find(subStrSteelGrade3);
			idx4=cstrSteelGrade.Find(subStrSteelGrade4);
			idx5=cstrSteelGrade.Find(subStrSteelGrade5);
			idx6=cstrSteelGrade.Find(subStrSteelGrade6);
			idx7=cstrSteelGrade.Find(subStrSteelGrade7);
			idx8=cstrSteelGrade.Find(subStrSteelGrade8);
			idx9=cstrSteelGrade.Find(subStrSteelGrade9);
			idx10=cstrSteelGrade.Find(subStrSteelGrade10);
			idx11=cstrSteelGrade.Find(subStrSteelGrade11);
			idx12=cstrSteelGrade.Find(subStrSteelGrade12);
			idx13=cstrSteelGrade.Find(subStrSteelGrade13);
			idx14=cstrSteelGrade.Find(subStrSteelGrade14);
			idx15=cstrSteelGrade.Find(subStrSteelGrade15);
			idx16=cstrSteelGrade.Find(subStrSteelGrade16);
			idx17=cstrSteelGrade.Find(subStrSteelGrade17);
			idx18=cstrSteelGrade.Find(subStrSteelGrade18);
			idx19=cstrSteelGrade.Find(subStrSteelGrade19);
			idx20=cstrSteelGrade.Find(subStrSteelGrade20);
			idx21=cstrSteelGrade.Find(subStrSteelGrade21);
			idx22=cstrSteelGrade.Find(subStrSteelGrade22);
			idx23=cstrSteelGrade.Find(subStrSteelGrade23);
			idx24=cstrSteelGrade.Find(subStrSteelGrade24);
			idx25=cstrSteelGrade.Find(subStrSteelGrade25);
			idx26=cstrSteelGrade.Find(subStrSteelGrade26);
			idx27=cstrSteelGrade.Find(subStrSteelGrade27);
			idx28=cstrSteelGrade.Find(subStrSteelGrade28);
			idx29=cstrSteelGrade.Find(subStrSteelGrade29);
			idx30=cstrSteelGrade.Find(subStrSteelGrade30);
			idx31=cstrSteelGrade.Find(subStrSteelGrade31);
		...
		else if (idx1!= -1|| idx2!= -1||idx3!= -1||idx4!= -1|| idx5!= -1|| idx6!= -1|| idx7!= -1)
		{
			...
			//SetWeight(fWeight-fTareWeight-0.55);//钢种名称包含：CH1T、CH1T-1、DT4、XG04XW、XGYT0、XGL1、CH1A,抛渣0.55t
			SetWeightNew(fWeight,fTareWeight,0.55);
		//SetWeight(fWeight - 1.35); //2009-03-13 PLC传送数据为净重 2009-04-16 钢水量为毛重-皮重-抛渣重量
		}
		else if(-1 != idx8 || -1!= idx9)
		{
			//SetWeight(fWeight-fTareWeight-0.75);//钢种名称包含：10B21-C、10B21-A,抛渣0.75t
			SetWeightNew(fWeight,fTareWeight,0.75);
		}
		else if(idx10 != -1|| idx11 != -1||idx12 != -1)
		{
			//SetWeight(fWeight-fTareWeight-0.47);//钢种名称包含：9Cr3Mo、9Cr2Mo、70Cr3Mo,抛渣0.47t
			SetWeightNew(fWeight,fTareWeight,0.47);
		}
		else if(idx13 != -1|| idx14 != -1 )
		{
			//SetWeight(fWeight-fTareWeight-0.59);//钢种名称包含：SWRH82B、SWRH77B,抛渣0.59t
			SetWeightNew(fWeight,fTareWeight,0.59);
		}
		else if(idx15 != -1)
		{
			//SetWeight(fWeight-fTareWeight-0.84);//钢种名称包含：GCr15,抛渣0.84t
			SetWeightNew(fWeight,fTareWeight,0.84);
		}
		else if(idx16 != -1 || idx17 != -1  )
		{
			//SetWeight(fWeight-fTareWeight-0.55);//钢种名称包含：SWRS82B、SWRS87B,抛渣0.55t
			SetWeightNew(fWeight,fTareWeight,0.55);
		}
		else if(idx18 != -1|| idx19 != -1 || idx20 != -1  || idx21 != -1 || idx29 != -1|| idx30 != -1||idx31 != -1 )
		{
			//SetWeight(fWeight-fTareWeight-0.7);//钢种名称包含：72A、82A、86A、92A、XGM6/XGYT1/XGDL,抛渣0.7t
			SetWeightNew(fWeight,fTareWeight,0.7);
		}
		else if((idx22 != -1 && cstrRHID == _T("S51")) || (idx23 != -1 && cstrRHID == _T("S51"))  || (idx24 != -1 && cstrRHID == _T("S51")) || (idx25 != -1 && cstrRHID == _T("S51")) )
		{
			//SetWeight(fWeight-fTareWeight-0.44);//过RH处理的，钢种名称包含：55SiCrA、55SiCrA-1、Q55SiCrA-1、60Si2Mn,抛渣0.44t
			SetWeightNew(fWeight,fTareWeight,0.44);
		}
		else if((idx22 != -1 && cstrRHID != _T("S51") && cstrSteelGradeIndex == _T("J1701010")) || (idx24 != -1 && cstrRHID != _T("S51") && cstrSteelGradeIndex == _T("J1710030"))  )
		{
			//SetWeight(fWeight-fTareWeight-0.78);//钢种：Q55SiCrA-1（JSKZ-138-70）--炼钢记号J1710030；或55SiCrA（JSKZ-138-1）-炼钢记号J1701010，工艺路线为BL，不过RH处理时,抛渣0.78t
			SetWeightNew(fWeight,fTareWeight,0.78);
		}
		else if(idx26 != -1 || idx27 != -1  || (idx28 != -1 && cstrRHID == _T("S51") && cstrSteelGradeIndex == _T("J1019000"))  )
		{
			//SetWeight(fWeight-fTareWeight-0.65);//钢种名称包含：SCM435-C、SCM435-A；或SCM435(Q/XG114)-炼钢记号J1019000，过RH处理时,抛渣0.65t
			SetWeightNew(fWeight,fTareWeight,0.65);
		}
		else
		{
			//SetWeight(fWeight-fTareWeight-0.8);//其它钢种,抛渣0.8t
			SetWeightNew(fWeight,fTareWeight,0.8);
		}
...
L3BOOL CLF_Unit_Mag::AcceptLFHeatGears(L3RECORDSET rsData)
{
	...
	if(strArea.IsEmpty() || (strArea !=_T("A") && strArea !=_T("B")))
	{
		//更改开始 20190821 当档位数据上传上来时，没有处理工位，不记录炉号，将数据记录到下一处理号
		rsGears.MoveFirst();
		while (!rsGears.IsEOF())
		{
			rsGears.SetFieldByName(_T("TreatNo"),CL3Variant(NextTreatNo));			
