...
L3BOOL CScrap_Store_Mag::ScrapOutStore(L3RECORDSET rsOutputInfo)
{
	...
	if(rs.GetFieldIndex(_T("MaterialType")) < 0 ||
		...
		rs.GetFieldIndex(_T("ScrapType")) < 0)
			return false;
     CString csTemp = rs.GetFieldByName(_T("ScrapType")).ToCString ();
	
	
	 OutputMaterials(rsOutputInfo);
  }
	CreateClassPropSet(SCRAP_OUT_STORE_DETAIL_CLASS_URI,rsData);
	/*{
		if(nTrans > 0)
			Rollback(nTrans);
		return FALSE;
       ...
       rsData.AddField(_T("MATERIAL_CODE"),VT_BSTR);
	   rsData.AddField(_T("AMOUNT"),VT_BSTR);
	   rsData.AddField(_T("MATERIAL"),VT_BSTR);
	   rsData.AddField(_T("Scrap_Slot_ID"),VT_BSTR);
	   rsData.SetFieldByName(_T("MATERIAL_CODE"),rs.GetFieldByName(_T("ScrapType")));
	  CString csTemp4 =  rsData.GetFieldByName(_T("MATERIAL_CODE")).ToCString ();
	  //CString csTemp5 =  rsOutputInfo.GetFieldByName(_T("ScrapType")).ToCString ();
	   rsData.SetFieldByName(_T("AMOUNT"),rs.GetFieldByName(_T("Amount")));
	   CString csTemp1 = rs.GetFieldByName(_T("ScrapType")).ToCString ();
	   CString csTemp2 = rs.GetFieldByName(_T("Amount")).ToCString ();
       CString csScrapType = rs.GetFieldByName (_T("ScrapType")).ToCString ();
	   L3LONG dAmout = rs.GetFieldByName(_T("Amount")).ToLong();
	   CString csStoreAreaID = rs.GetFieldByName(_T("StoreAreaID")).ToCString ();
	   if (csStoreAreaID == "S82A01")
	   {
		   rsData.SetFieldByName(_T("MATERIAL"),CL3Variant(_T("ScrapGroupName")));
	   }
	   else if (csStoreAreaID == "S82A02")
	   {
		   rsData.SetFieldByName(_T("MATERIAL"),CL3Variant(_T("PigIronGroupName")));
	   }
	LONG csTemp3 =  rsData.GetRowCount ();
 ...
 //   CString csMaterial_Code1 = rsData.GetFieldByName(_T("MATERIAL_CODE")).ToCString();
	//rsData.MoveFirst();
	//while(!rsData.IsEOF())
	//{
	//	CString csMaterial_Code = rsData.GetFieldByName(_T("MATERIAL_CODE")).ToCString();
	//	L3LONG dAmout = rsData.GetFieldByName(_T("AMOUNT")).ToLong();
	//	CString csMaterial = rsData.GetFieldByName(_T("MATERIAL")).ToCString();

	//	if(csMaterial_Code==_T("")||dAmout<0||csMaterial==_T(""))
	//		return -1;
	//	rsData.SetFieldByName(_T("Scrap_Slot_ID"),(CL3Variant)csScrap_Slot_ID);
	...
	//}
	//
	//CreateNewObjects(SCRAP_OUT_STORE_DETAIL_CLASS_URI,rsData);
	//if(bUseTrans)
	//	Commit(nTrans);
	//return 0;
	  SaveScrapOutDetail(rsData);
}

/// <Method class="CScrap_Store_Mag" name="ScrapOutStoreNew" type="L3VOID">
/// 
/// 废钢出库
/// ARG&gt;&gt; rsOutputInfo废钢生铁信息.必须包含MaterialType, MaterialID, Amount, StoreAreaID,Scrap_Slot_ID,Car_ID,Car_Type,Net_Weight,Gross_Weight,Tare_Weight这几个字段。
/// RET &lt;&lt; 成功返回TRUE；失败返回FALSE。
/// <Param name="rsOutputInfo" type="L3RECORDSET">出库信息记录。包含MaterialType,MaterialID,Amount,AreaID等字段。</Param>
/// </Method>
L3BOOL CScrap_Store_Mag::ScrapOutStoreNew(L3RECORDSET rsOutputInfo)
{
	CL3RecordSetWrap rs =rsOutputInfo;
	if( rs.IsNull() || rs.GetRowCount() < 1)
		return false;
	...
	/*if(rs.GetFieldIndex(_T("MaterialType")) < 0 ||
     ...
     CString csTemp = rs.GetFieldByName(_T("ScrapType")).ToCString ();*/
	   ...
	   CreateClassPropSet(SCRAP_OUT_CLASS_URI,rslog);
	   rslog.AppendRecord();
	   rslog.MoveFirst();
	   rslog.AddField (_T("MaterialType"),VT_BSTR);
	   rslog.AddField (_T("MaterialID"),VT_BSTR);
	   rslog.AddField (_T("Amount"),VT_BSTR);
	   rslog.AddField (_T("Scrap_Slot_ID"),VT_BSTR);
	   rslog.AddField (_T("StoreAreaID"),VT_BSTR);
       rs.MoveFirst ();
	L3DOUBLE lfgWeight = rs.GetFieldByName (_T("fgWeight")).ToFloat ();
	L3DOUBLE lstWeight = rs.GetFieldByName (_T("stWeight")).ToFloat ();
	
		rslog.SetFieldByName (_T("MaterialType"),rs.GetFieldByName (_T("MaterialType")));
		rslog.SetFieldByName (_T("MaterialID"),CL3Variant(_T("PigIron")));
		rslog.SetFieldByName (_T("Amount"),rs.GetFieldByName (_T("stWeight")));
		//rslog.SetFieldByName (_T("Amount"),L3LONG(22));
		rslog.SetFieldByName (_T("Scrap_Slot_ID"),rs.GetFieldByName (_T("Scrap_Slot_ID")));
		rslog.SetFieldByName (_T("StoreAreaID"),CL3Variant(_T("S82A02")));
	
	
	 OutputMaterials(rslog);

	 CString csSql;
	csSql.Format(_T("select SCRAP_SLOT_ID,OPERATOR,SHIFT,TEAM,IRON_FLAG,LOGDATE from %s where LOGDATE>sysdate-0.007 and STOREAREAID='S82A02' order by LOGDATE desc "),SCRAP_OUT_CLASS_URI);
	...
	CL3RecordSetWrap rspigiron;
	CreateClassPropSet(SCRAP_OUT_CLASS_URI,rspigiron);
	rspigiron.AppendRecord();
	rspigiron.AddField (_T("Scrap_Slot_ID"),VT_BSTR);
	rspigiron.AddField (_T("MaterialType"),VT_BSTR);
    rspigiron.AddField (_T("MaterialID"),VT_BSTR);
	rspigiron.AddField (_T("Amount"),VT_BSTR);
	rspigiron.AddField (_T("AreaID"),VT_BSTR);
	rspigiron.AddField (_T("Shift"),VT_BSTR);
    rspigiron.AddField (_T("Team"),VT_BSTR);
	rspigiron.AddField (_T("Iron_Flag"),VT_BSTR);
	rspigiron.AddField (_T("Operator"),VT_BSTR);
	rspigiron.AddField (_T("LogDate"),VT_DATE);
	rspigiron.SetFieldByName (_T("Scrap_Slot_ID"),rsScrapData.GetFieldByName (_T("Scrap_Slot_ID")));
	rspigiron.SetFieldByName (_T("MaterialType"),CL3Variant(_T("CScrap_Data")));
	rspigiron.SetFieldByName (_T("MaterialID"),CL3Variant(_T("Scrap")));
	rspigiron.SetFieldByName (_T("Amount"),rs.GetFieldByName (_T("fgWeight")));
	//rspigiron.SetFieldByName (_T("Amount"),L3LONG(33));
	rspigiron.SetFieldByName (_T("StoreAreaID"),CL3Variant(_T("S82A01")));
	rspigiron.SetFieldByName (_T("Shift"),rsScrapData.GetFieldByName (_T("Shift")));
	rspigiron.SetFieldByName (_T("Team"),rsScrapData.GetFieldByName (_T("Team")));
	rspigiron.SetFieldByName (_T("Iron_Flag"),rsScrapData.GetFieldByName (_T("Iron_Flag")));
	rspigiron.SetFieldByName (_T("Operator"),rsScrapData.GetFieldByName (_T("Operator")));
	rspigiron.SetFieldByName (_T("LogDate"),rsScrapData.GetFieldByName (_T("LogDate")));
	CreateNewObjects(SCRAP_OUT_CLASS_URI,rspigiron);

  }
	 for ( int i=5;i<46;i++ )
	 {
		 L3FLOAT iTemp ;
		 iTemp = rs.GetFieldByIndex(i).ToFloat ();
		 CL3RecordSetWrap rsData;
		 CreateClassPropSet(SCRAP_OUT_STORE_DETAIL_CLASS_URI,rsData);
		 rsData.AppendRecord();
		 rsData.MoveFirst();
		 ...
		 if (iTemp > 0 && i < 35)
		 {
			 CString temp1;
			 CString temp2;
			 temp1.Format(_T("%d"),( i - 4));
			 temp2 =  _T("fg")+ temp1;
			 
			 rsData.SetFieldByName(_T("MATERIAL_CODE"),CL3Variant(temp2));
			 //rsData.SetFieldByName(_T("AMOUNT"),rs.GetFieldByName(_T("Amount")));
			 rsData.SetFieldByName(_T("AMOUNT"),CL3Variant((FLOAT)iTemp));
			 rsData.SetFieldByName(_T("MATERIAL"),CL3Variant(_T("ScrapGroupName")));
		 }
		 else if (iTemp > 0 && i > 34 )
		 {
			 CString temp1;
			 CString temp2;
			 temp1.Format(_T("%d"),( i - 34));
			 temp2 =  _T("st")+ temp1;
			 rsData.SetFieldByName(_T("MATERIAL_CODE"),CL3Variant(temp2));
			 //rsData.SetFieldByName(_T("AMOUNT"),rs.GetFieldByName(_T("Amount")));
			 rsData.SetFieldByName(_T("AMOUNT"),CL3Variant((FLOAT)iTemp));
...
/// <Method class="CScrap_Store_Mag" name="GetMaterialOnStore" type="L3RECORDSET">
/// 
/// 获取库区材料信息
/// ARG&gt;&gt; 
/// RET &lt;&lt;返回数据集
...
/// <Method class="CScrap_Store_Mag" name="GetMaterialOnStore1" type="L3RECORDSET">
/// 
/// 获取二库区材料信息
/// ARG&gt;&gt; 
/// RET &lt;&lt;返回数据集
/// </Method>
L3RECORDSET CScrap_Store_Mag::GetMaterialOnStore1(L3STRING strStoreID)
{
	CString csPos = strStoreID;
	CString csTemp;
	CString csSQL;
	...
	{
		csTemp= _T(" select a.*  from %s a  where  substr(a.Position,1,3) = '%s' and  substr(a.Position,4,1) <> 'P' and substr(a.Position,4,1) <> 'N'");
		csSQL.Format(csTemp,MES_MATERIAL_POSITION,STORE_ID_SCRAP1);
	}
	else
	{
		csTemp = _T(" select a.*  from %s a  where  a.Position = '%s' ");
...
/// <Param name="rsInputInfo" type="L3RECORDSET"></Param>
/// </Method>
L3BOOL CScrap_Store_Mag::ScrapCancelOutStore1(L3RECORDSET rsOutputInfo)
{
	...
	{
		CString csCarID = rsData.GetFieldByName(_T("Scrap_Slot_ID")).ToCString();
		CString csTempSQL =_T(" select GUID,MaterialType,MaterialID,Amount,StoreAreaID as TargetID,'S84N01'as SourceID  from CScrap_Output_Log where Iron_flag =0 and Scrap_Slot_ID ='%s'");
		CString csSQL;
		csSQL.Format(csTempSQL,csCarID);
		CL3RecordSetWrap rsResultData;
		if(!Query(csSQL,rsResultData))
...
L3RECORDSET CScrap_Store_Mag::GetScrapes()
{
	...
	cssss += _T(" sum(Amount)-sum(decode(StoreAreaID,'S82A01',Amount,0)) as PigIronWT ,to_char(LogDate,'yyyy-mm-dd') as LogTime from CScrap_Output_Log where Iron_Flag = 0  AND (StoreAreaID ='S82A01' OR StoreAreaID ='S82A02') group by Scrap_Slot_ID,to_char(LogDate,'yyyy-mm-dd')");
	CString strSQL;
...
/// 获取可选废钢信息记录。返回带有废钢槽号和废钢量的记录集。
/// </Method>
L3RECORDSET CScrap_Store_Mag::GetScrapes1()
{
	//2009-02-21*Liao
	CString cssss = _T("select Scrap_Slot_ID,sum(Amount) as Amount,sum(decode(StoreAreaID,'S84A01',Amount,0)) as ScrapWT,");
	cssss += _T(" sum(Amount)-sum(decode(StoreAreaID,'S84A01',Amount,0)) as PigIronWT ,to_char(LogDate,'yyyy-mm-dd') as LogTime from %s where  Iron_Flag = 0  AND (StoreAreaID ='S84A01' OR StoreAreaID ='S84A02') group by Scrap_Slot_ID,to_char(LogDate,'yyyy-mm-dd')");
	CString strSQL;
	//strSQL.Format(_T("select Scrap_Slot_ID,sum(Amount) as Amount sum(decode(StoreAearID))from %s where Iron_Flag = 0 group by Scrap_Slot_ID"),SCRAP_OUT_CLASS_URI);
	strSQL.Format(cssss,SCRAP_OUT_CLASS_URI);
...
/// 保存二库废钢、生铁分类重量信息
/// </Method>2011-03-19 用户需要记录废钢种类信息添加
L3LONG CScrap_Store_Mag::SaveScrapOutDetail1(L3RECORDSET rsScrap)
{
	// TODO : 请在此添加逻辑代码
	CL3RecordSetWrap pScrap =rsScrap;
	if( pScrap.IsNull() || pScrap.GetRowCount() < 1)
	...
	//获取最后一次料斗出库记录的料斗号
	CString csSql;
	csSql.Format(_T("select SCRAP_SLOT_ID from %s where LOGDATE>sysdate-0.007 and STOREAREAID='S84A01' order by LOGDATE desc "),SCRAP_OUT_CLASS_URI);
