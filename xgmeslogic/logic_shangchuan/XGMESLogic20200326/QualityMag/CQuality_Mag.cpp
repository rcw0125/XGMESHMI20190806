...
L3RECORDSET CQuality_Mag::GetSampleApplyData(L3STRING strSampleAddr)
{
	...
	//增加熔化炉
		else if (cstrSampleAddr.Left(1) == _T("E"))
	{
		cstrSQL += _T("  and Sample_Address = '%s' ");
		cstrSQL += _T("  and ((Back_Flag = 1) or (SendFlag = 0) or (ReSample_Flag = 1)) order by Sample_Date desc ");
...
L3RECORDSET CQuality_Mag::GetHeatIDByAddr(L3STRING strSampleAddr)
{
	...
	{
		strSQL.Format(_T("select HeatID from (select * from CMELT_Base_Data  order by productiondate desc) where rownum < 5"));
	}
	...
	else if (cstrSampleAddr.Left(1) == _T("H"))
	{
		strSQL.Format(_T("select MATERIALID from (select * from CIRON_DATA WHERE HEATID IS NULL order by TAP_TIME desc) where rownum < 4")
			,cstrSampleAddr.Right(1));
	}
	CL3RecordSetWrap rs;
	if(!Query(strSQL,rs))
