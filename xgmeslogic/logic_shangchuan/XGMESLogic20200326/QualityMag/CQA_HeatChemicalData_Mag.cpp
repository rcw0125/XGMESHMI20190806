...
/// <Param name=无</Param>
/// </Method>
CString CQA_HeatChemicalData_Mag::GetSysCurYearNew()
{
	COleDateTime dtCurrentTime;
	dtCurrentTime = COleDateTime::GetCurrentTime();
	CString cstrCurTime = dtCurrentTime.Format(_T("%Y"));
    CString cstrYear = cstrCurTime;
    return cstrYear;
}
...
/// <Method class="CQA_HeatChemicalData_Mag" name="CalIronLadleSampleCode" type="CString">
/// 计算转炉铁包样编码 add by yuan 2018-07-02
/// <Param name="csSampleAddr" type="LPCTSTR">取样地点(混铁炉)</Param>
/// <Param name="csSampleType" type="LPCTSTR">试样类别</Param>
/// <Param name="csIronLadleNo" type="LPCTSTR">铁包</Param>
/// </Method>
CString  CQA_HeatChemicalData_Mag::CalBofIronLadleSampleCode(LPCTSTR csSampleAddr,LPCTSTR csSampleType,LPCTSTR csIronLadleNo)
{
	//[20090106Liao]
    CString cstrSampleAddr = csSampleAddr;
	CString cstrIronLadleID = csIronLadleNo;
	CString cstrSampleType = csSampleType;
	//cstrIronLadleID = cstrIronLadleID.Left(3);
	...
	//CString cstrSampleNo=_T("B") + cstrIronLadleID + cstrcurDay  ;
    /*CString cstrSampleNo=_T("B") + cstrIronLadleID ;*/
	CString cstrSampleNo= cstrIronLadleID ;
	CL3RecordSetWrap rs;
    //从CQA_Lab_Sheet_App获取混铁炉当天最大取样次数，如果为空当天取样次数为0
	CString csSQL;
	
	/*csSQL.Format(_T("select Max(Sample_Count) AS Sample_Count from %s where Sample_Address = '%s' and sample_type = '%s' and to_char(Sample_Date,'yyyyMMdd')='%s' "),
		QA_LAB_SHEET_APP_CLASS_URI,csSampleAddr,cstrSampleType,cstrcurDay);*/
	csSQL.Format(_T("select Max(Sample_Count) AS Sample_Count from %s where sample_type = '%s' and to_char(Sample_Date,'yyyyMMdd')='%s' "),
		QA_LAB_SHEET_APP_CLASS_URI,cstrSampleType,cstrcurDay);
	if(!Query(csSQL,rs))
		return _T("");
    ...
    //return _T("B")+ cstrcurDay + cstrIronLadleID +cstrSampleCount;
	/*return _T("B") + cstrIronLadleID +cstrSampleCount;*/
	/*return cstrIronLadleID.Mid(0,3) + cstrcurDay.Right(6) +cstrSampleCount;*/
	//return cstrIronLadleID +cstrSampleCount;
	/*return cstrIronLadleID.Mid(0,12) +cstrSampleCount;*/
	return cstrIronLadleID.Mid(0,3) + cstrIronLadleID.Mid(12,6) +cstrSampleCount;
}
...
L3LONG  CQA_HeatChemicalData_Mag::NewLabSheet(L3RECORDSET rsData)
{
	...
	while(!rs.IsEOF())
	{
		...
		else if (cstrSample_Type == QA_Sample_Type_TL) // 转炉铁包样（2018-07-02）
		{
			cstrSampleCode = CalBofIronLadleSampleCode(cstrSample_Address,cstrSample_Type,cstrHeatID);//cstrHeatID为铁包
		}
		else if ((cstrSample_Type == QA_Sample_Type_TC) && (cstrSample_Address == QA_Sample_Addr_TC)) //铁水抽查样
		{
			...
			else if ((cstrSample_Type == _T("TP")) || (cstrSample_Type == _T("TI")) || (cstrSample_Type == _T("ZP"))) //脱磷铁水抽查样 20160316
		{
			cstrSampleCode = CalIronSampleCode(cstrSample_Address,cstrSample_Type,cstrHeatID);//cstrHeatID为铁次号
		}
...
L3LONG CQA_HeatChemicalData_Mag::AcceptLabIronElement(L3RECORDSET rsData)
{
	...
	{
		CString cstrSampleAddr = rsElement.GetFieldByName(_T("Sample_Address")).ToCString();
		CString cstrSampleCode = rsElement.GetFieldByName (_T("Sample_Code")).ToCString ();
		CString cstrUnitType = cstrSampleAddr.Mid(0,1);
		CString cstrUnitID = cstrSampleAddr.Mid(1,1);
		if (cstrUnitType == _T("G")) //高炉铁沟样
		{
		...
		else if (cstrUnitType == _T("H")) //混铁炉区域样
		{
			...
			//CString csSql;
			//csSql.Format(_T("select materialid from ciron_data where tap_time > sysdate-0.007 order by tap_time desc  "),MATERIAL_IRON_URI);

			//CL3RecordSetWrap rsIronData;
			//if(!Query(csSql,rsIronData)||rsIronData.GetRowCount()<1)
			//	return -1;

			////取最后一条数据
			//rsIronData.MoveFirst();
			//CString csMaterialid=rsIronData.GetFieldByName(_T("MaterialID")).ToCString();
			////设置转炉铁包号
			//rsElement.SetFieldByName(_T("TAP_No"),CL3Variant(csMaterialid));
			/////end add by yuan 2018-06-21
			...
			//Modify end
			{
				...
				//CString csCIronDataURI = _T("MES\\MaterialData\\CIron_Data")+csMaterialid;//add by yuan 2018-06-21
				...
				if(!(valC.IsEmpty() || valC.IsError() || valC.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("C"),valC);//add by yuan 2018-06-21
				}
				...
				if(!(valSi.IsEmpty() || valSi.IsError() || valSi.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Si"),valSi);//add by yuan 2018-06-21
				}
				...
				if(!(valMn.IsEmpty() || valMn.IsError() || valMn.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Mn"),valMn);//add by yuan 2018-06-21
				}
				...
				if(!(valP.IsEmpty() || valP.IsError() || valP.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("P"),valP);//add by yuan 2018-06-21
				}
				...
				if(!(valS.IsEmpty() || valS.IsError() || valS.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("S"),valS);//add by yuan 2018-06-21
				}
				...
				if(!(valTi.IsEmpty() || valTi.IsError() || valTi.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Ti_1"),valTi);//add by yuan 2018-06-21
				}
				...
				if(!(valSn.IsEmpty() || valSn.IsError() || valSn.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Sn_1"),valSn);//add by yuan 2018-06-21
				}
				...
				if(!(valSb.IsEmpty() || valSb.IsError() || valSb.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Sb_1"),valSb);//add by yuan 2018-06-21
				}
				...
				if(!(valAs.IsEmpty() || valAs.IsError() || valAs.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("As_1"),valAs);//add by yuan 2018-06-21
				}
				...
				if(!(valPb.IsEmpty() || valPb.IsError() || valPb.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Pb_1"),valPb);//add by yuan 2018-06-21
				}
				//end
				...
				if(!(valNi.IsEmpty() || valNi.IsError() || valNi.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Ni_1"),valNi);//add by yuan 2018-06-21
				}
				...
				if(!(valMo.IsEmpty() || valMo.IsError() || valMo.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Mo_1"),valMo);//add by yuan 2018-06-21
				}
				...
				if(!(valCu.IsEmpty() || valCu.IsError() || valCu.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Cu_1"),valCu);//add by yuan 2018-06-21
				}
				...
				if(!(valCr.IsEmpty() || valCr.IsError() || valCr.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Cr_1"),valCr);//add by yuan 2018-06-21
				}
				...
				if(!(valB.IsEmpty() || valB.IsError() || valB.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("B_1"),valB);//add by yuan 2018-06-21
				}
				...
				if(!(valV.IsEmpty() || valV.IsError() || valV.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("V_1"),valV);//add by yuan 2018-06-21
				}
				...
				if(!(valAl.IsEmpty() || valAl.IsError() || valAl.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Al_1"),valAl);//add by yuan 2018-06-21
				}
				...
				if(!(valNb.IsEmpty() || valNb.IsError() || valNb.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Nb_1"),valNb);//add by yuan 2018-06-21
				}
				...
				if(!(valZn.IsEmpty() || valZn.IsError() || valZn.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("Zn_1"),valZn);//add by yuan 2018-06-21
				}
				...
				if(!(valW.IsEmpty() || valW.IsError() || valW.IsNull()))
				{
					...
					//SetObjectPropValue(csCIronDataURI,_T("W_1"),valW);//add by yuan 2018-06-21
				}
				//end 将元素后缀名加上 _ELE
			}
			if (cstrSampleType == _T("TL")) //若混铁炉样则设置混铁炉加工区域最新成份值
			{
				CString cstrSubSampleCode = rsElement.GetFieldByName (_T("Sample_Code")).ToCString ();
				//CString csMaterialid = cstrSubSampleCode.Mid(0,18);
				CString cstrYear = GetSysCurYearNew();
				COleDateTime dtcurDay =  COleDateTime::GetCurrentTime();
				CString cstrcurDay = dtcurDay.Format(_T("%m%d"));
				cstrcurDay = cstrYear + cstrcurDay;
				CString csMaterialid = cstrSubSampleCode.Mid(0,3)+ _T("_") + cstrcurDay + cstrSubSampleCode.Mid(3,6) ;
				CString csCIronDataURI = _T("MES\\MaterialData\\CIron_Data\\")+csMaterialid;//add by yuan 2018-06-21
				CL3Variant valC = rsElement.GetFieldByName(_T("C"));
				if(!(valC.IsEmpty() || valC.IsError() || valC.IsNull()))
				{
