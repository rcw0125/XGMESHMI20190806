...
L3BOOL CBOF_Unit_Mag::FeedIrons()
{
	...
	rsProc.SetFieldByName(_T("RemainSlag"),CL3Variant(_T("0")));
...
L3LONG CBOF_Unit_Mag::DesignateMainMaterial(L3RECORDSET rsIrons,L3RECORDSET rsReturnSteel,L3RECORDSET rsScrapes)
{
	...
	{	
		...
		//ciron表中的tare 记录压块 20190812
		double fTare = rsIronsDesignated.GetFieldByName(_T("Tare")).ToDouble();//2009-03-09
	...
	while(!rsFeedFirData.IsEOF())
	{
		...
		double fYkWeight = rsFeedFirData.GetFieldByName(_T("YkWeight")).ToDouble();
		Temp += fTmp;
		C += fW * fC;
		Si += fW * fSi;
		Mn += fW * fMn;
	...
	//添加压块 20190813
	rsFeedData.SetFieldByName(_T("YkWeight"),CL3Variant(YkWeight));
	...
	//end
	//update by rcw 2019-01-18  rsFeedData.SetFieldByName(_T("Cr"),CL3Variant(Pb));
    rsFeedData.SetFieldByName(_T("Cr"),CL3Variant(Cr));
	COleDateTime curTime = COleDateTime::GetCurrentTime();
	rsFeedData.SetFieldByName(_T("LogTime"),CL3Variant(curTime));
	rsFeedData.SetFieldByName(_T("IronTemp"),CL3Variant(Temp));

	Load = IronWeight + ScrapeWeight + PigIronWeight + Return_Steel_Weight  + YkWeight;//装入量
	rsFeedFirData.MoveFirst();
...
L3BOOL CBOF_Unit_Mag::StartTapping()
{
    ...
    CalculateLcTemp();//计算拉碳温度 20190911
...
L3BOOL CBOF_Unit_Mag::CaculateLadleGrade(L3DATETIME dtTapTime,L3STRING strHeatID)
{
	...
	while(!rsLadle.IsEOF())
	{
		...
		else
		{
			...
			CString csSlagWire =  rsLadle.GetFieldByName(_T("SlagWire")).ToCString(); 
			L3LONG iWall_Temp =  rsLadle.GetFieldByName(_T("Wall_Temp")).ToLong();
			...
			if(csLadleGrade_Cal == csLadleGrade_Act)
			{
				switch(nLadleType)
				{
					...
					break;
					case 2: //50吨精炼钢包
						if(nTime<=1.5*60 && csSlagWire == "") 
							csLadleGrade_Act =_T("A1");
                        else
							if((1.5*60< nTime) && (nTime<=2.5*60) && csSlagWire == "")
								csLadleGrade_Act =_T("A2");
						else
							if((2.5*60< nTime) && (nTime<=3.5*60) && csSlagWire == "")
								csLadleGrade_Act =_T("B1");
						else
							if((3.5*60< nTime) && (nTime<=4.5*60) && csSlagWire == "")
								csLadleGrade_Act =_T("B2");
						else
							if((nTime<=4.5*60) && csSlagWire != "")
								csLadleGrade_Act =_T("B3");
						else
								if((4.5*60< nTime) && (nTime <=5.5*60) && iWall_Temp >= 600)
									csLadleGrade_Act =_T("C1");
						else
								if((5.5*60< nTime) && (nTime <=6.5*60) && iWall_Temp >= 600)
									csLadleGrade_Act =_T("C2");
						else
								if((nTime > 6.5*60) && iWall_Temp > 600)
									csLadleGrade_Act =_T("D1");

					    else    if(iWall_Temp < 600 && iWall_Temp > 0 )
									csLadleGrade_Act =_T("D2");
					break;
					case 3: //80吨精炼钢包
						if(nTime <=2*60 && csSlagWire == "") 
							csLadleGrade_Act =_T("A1");
						else
							if((2*60< nTime) && (nTime<=4*60) && csSlagWire == "")
								csLadleGrade_Act =_T("A2");
						else
							if((4*60< nTime) && (nTime<=6*60) && csSlagWire == "")
								csLadleGrade_Act =_T("B1");
						else
							if((6*60< nTime) && (nTime<=8*60) && csSlagWire == "")
								csLadleGrade_Act =_T("B2");
						else
							if((nTime<=8*60) && csSlagWire != "")
								csLadleGrade_Act =_T("B3");
						else
								if((8*60< nTime) && (nTime <=10*60) && iWall_Temp >= 600)
									csLadleGrade_Act =_T("C1");
						else
								if((10< nTime) && (nTime <=12*60) && iWall_Temp >= 600)
									csLadleGrade_Act =_T("C2");
						else
								if((nTime > 12*60) && iWall_Temp > 600)
									csLadleGrade_Act =_T("D1");
...
L3BOOL CBOF_Unit_Mag::CalculateLcTemp()
{
	...
	//拉碳温度为本炉的第一温度20190911
    CString csSQL;
	CString strUri = CL3NameParser::MergeClassDomainName(BOF_PROCESS_DATA_CLASS_URI,curHeatID);
	csSQL.Format(_T("select Temp from %s where HeatID = '%s' order by Temp_Time "),BOF_TEMP_DATA_CLASS_URI,curHeatID);
	CL3RecordSetWrap rstemp;
...
L3BOOL CBOF_Unit_Mag::CalculateTappingTemp()
{
   ...
   //Modify begin by llj 2011-0-09 修正拉碳温度为喷吹开始6分钟后的第一个温度   20190911 屏蔽
...
L3BOOL CBOF_Unit_Mag::CalculateDefaultSmeltPeriod()
{
    ...
    CString strUri1 = CL3NameParser::MergeClassDomainName(BOF_PROCESS_DATA_CLASS_URI,curHeatID);
	...
	LONG smeltperious = (LONG)span.GetTotalSeconds();

	if (smeltperious > 0)
	{
	   if (!SetObjectPropValue(strUri1,_T("Smelt_Period"),CL3Variant((L3LONG)smeltperious)))
	   {
			return FALSE; 
	   }
	}
...
L3BOOL CBOF_Unit_Mag::AcceptBOFRemainSlag(L3RECORDSET rsData)
{
    ...
    //将 CString csheatslag =  GetObjectPropValue(csHeatIDUri,_T("RemainSlag")).ToCString();
	//改为sql查询方法
	CString csSQL;
    csSQL.Format(_T("select RemainSlag  from CBOF_Process_Data where heatid ='%s'"),csHeatID);
	CL3RecordSetWrap rsTemp;
	if(!Query(csSQL,rsTemp) || rsTemp.IsNull() || rsTemp.GetRowCount() < 1)
		return FALSE;
	rsTemp.MoveFirst();
	CString csheatslag = rsTemp.GetFieldByName(_T("RemainSlag")).ToCString();

   // CString csheatslag =  GetObjectPropValue(csHeatIDUri,_T("RemainSlag")).ToCString();
	//当前炉次还没有判定
	if(csheatslag=="0")
	{
		...
		CL3RecordSetWrap rsStatus;
		if(!CreateClassPropSet(_T("XGMESLogic\\BOFMag\\CBOF_Process_Slag"),rsStatus))
		{			
		   return FALSE;
	    }
	    rsStatus.AppendRecord();
		/*rsStatus.SetFieldByName(_T("Name"),CL3Variant("1"));*/
        rsStatus.SetFieldByName(_T("HeatID"),CL3Variant(csHeatID));
		 if(nRemainSlag > 0)
		{	
		    rsStatus.SetFieldByName(_T("Name"),CL3Variant("否"));
		}
		else if(nRemainSlag < 0)
		{
			rsStatus.SetFieldByName(_T("Name"),CL3Variant("是"));
		}
	    rsStatus.SetFieldByName(_T("Change_Time"),CL3Variant(COleDateTime::GetCurrentTime()));
	    if(CreateNewObjects(_T("XGMESLogic\\BOFMag\\CBOF_Process_Slag"),rsStatus) < 1)
	    {		
		 return FALSE;
	    }
	}
	//已经判定了，说明MES维护时间晚，实际已开吹，记录状态
	else
	{
	  //计算炉次号，当前炉号为下一炉的炉号
	    CString curheatid=  CalculateNextHeatID(csHeatID);
		CL3RecordSetWrap rsStatus;
		if(!CreateClassPropSet(_T("XGMESLogic\\BOFMag\\CBOF_Process_Slag"),rsStatus))
		{			
		   return FALSE;
	    ...
	    rsStatus.AppendRecord();
        rsStatus.SetFieldByName(_T("HeatID"),CL3Variant(curheatid));
		rsStatus.SetFieldByName(_T("Status"),CL3Variant(-1));
	     if(nRemainSlag > 0)
		{	
		    rsStatus.SetFieldByName(_T("Name"),CL3Variant("否"));
		}
...
/// <Method class="CBOF_Unit_Mag" name="AcceptBOFN2Pressure" type="L3BOOL">
/// 接收转炉溅渣N2压力数据 2018-05-16
/// <Param name="rsData" type="L3RECORDSET">留渣数据</Param>
/// </Method>
L3BOOL CBOF_Unit_Mag::AcceptBOFN2Pressure(L3RECORDSET rsData)
{
	CL3RecordSetWrap rs = rsData;
	if(rs.GetRowCount() < 1)
		return TRUE;
	...
	rs.MoveFirst();
	CString csHeatID = HeatID;
	CString csHeatIDUri = BOF_PROCESS_DATA_CLASS_URI + _T("\\") + csHeatID;
    L3FLOAT fN2Pressure = rs.GetFieldByName(_T("Nitrogen_Pressure")).ToFloat();
	SetObjectPropValue(csHeatIDUri,_T("N2Pressure"),CL3Variant(fN2Pressure));
	...
	rs.MoveFirst();
	CString csHeatID =GetPropValue(_T("HeatID")).ToCString();
	CString strBOFID = rs.GetFieldByName(_T("BOFID")).ToCString();
    L3FLOAT fN2Pressure = rs.GetFieldByName(_T("Nitrogen_Pressure")).ToFloat();
