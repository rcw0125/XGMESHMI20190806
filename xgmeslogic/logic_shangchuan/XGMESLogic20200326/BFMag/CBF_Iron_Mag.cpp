...
L3LONG CBF_Iron_Mag::DeleteIronData(L3RECORDSET rsIron)
{
	...
	{
		L3LONG nStatus = rs.GetFieldByName(_T("IronStatus")).ToLong();
        L3DOUBLE fNetWeight = rs.GetFieldByName(_T("Net_Weight")).ToDouble();
		if(fNetWeight > 0)
		{
			if(nStatus != CBFIron::ironNotProcessed  )
			{
...
{
	CString cstrID = strTpcNo;
	CString csTapNo =cstrID.Left(9);

	CString strSQL;
	strSQL.Format(_T("select * from weight.tb_ironinfo where tpc_No = '%s' and (sysdate - to_date(CREATETIME,'yyyy-mm-dd hh24:mi:ss')) <=5/24 "),cstrID);
	CL3RecordSetWrap rs;
	if(Query(strSQL,rs) || !rs.IsNull() || rs.GetRowCount() > 0)
		return FALSE;
	//组建数据集
