...
L3RECORDSET CPlan_Mag::GetCastingPlan(L3STRING strDate,L3STRING strCasterID)
{
	...
	CString csDate = strDate;
	CString csTemp = _T(" SELECT a.*,a.Thickness || '*' || a.Width || '*' || a.Length as Std ,b.protocol,b.ProduceStd,b.TECH_REQUEST,a.materialcode                   ");
...
/// </Method>
L3LONG CPlan_Mag::GenCastingPlanOfSpecifyCasterID(CString csCasterID,CString csMaterialCode,L3RECORDSET rsPlanOrder,
												  ...
												  L3DOUBLE fWeight,L3SHORT nBOFFlag)
{
	...
	for(int i=0;i<nHeatCount;i++)
	{
		...
		rsCastingPlan.SetFieldByName(_T("MaterialCode"),rsPlanOrd.GetFieldByName(_T("MaterialCode")));
		...
		CString csTemp;
		//更据铸机号获取最优工艺路径
		CString csMaterialCodeTemp = csMaterialCode.Mid(0,3);
		if(csMaterialCode.Mid(0,3) == "617")
		{
			 csTemp = _T("Select * from %s where CasterID = '%s' and Lev_Type = 1 and bofid='S51'");
		}
		else
		{
			 csTemp = _T("Select * from %s where CasterID = '%s' and Lev_Type = 1 ");
		}
		CString csSQL;
		...
		//得到AOD炉座
		CL3Variant vsAODID = rsRoute.GetFieldByName(_T("AODID"));
		if(!vsAODID.IsValid() || vsAODID.IsError() || vsAODID.IsNull())
		{
			if(bUseTrans)
				Rollback(nTrans);
			return -99;
		}
		CString csAODID = vsAODID.ToCString();

		//获取AOD区分
		CL3Variant vsAODType = GetObjectPropValue(QA_PROD_SHEET_CLASS_URI + _T("\\") + csSteelGradeIndex,_T("AOD_Type"));
		if(!vsAODType.IsValid() || vsAODType.IsError() || vsAODType.IsNull())
		{
			if(bUseTrans)
				Rollback(nTrans);
			return -93;
		}
		CString csAODType = vsAODType.ToCString();
		//获取精炼区分
		...
		if(csLFType != _T("01") && csRHType == _T("01") && csAODType != ("01"))//经过LF，经过AOD，但不经过RH
		{
			//if(csCasterID == _T("S65")) csLFID = _T("S43");//铸机是5#铸机，必须经过3#LF
			//if((csLFID == _T("0") || csLFID == _T("S43")) && csCasterID != _T("S65")) csLFID = _T("S41");//LF未确定或者3#LF，并且是5#铸机，必须经过1#LF
			rsCastingPlan.SetFieldByName(_T("LFID"),CL3Variant(csLFID));
			rsCastingPlan.SetFieldByName(_T("RHID"),CL3Variant(_T("0")));
			rsCastingPlan.SetFieldByName(_T("AODID"),CL3Variant(csAODID));
			rsCastingPlan.SetFieldByName(_T("Refine_Type"),CL3Variant(_T("A")));
		}
...
L3LONG CPlan_Mag::CastingPlanGen(L3RECORDSET rsPlan)
{
	...
	while(!rsPlanOrd.IsEOF())
	{
		...
		L3SHORT nBOFFlag = 0;
		L3LONG	nBOFType = GetObjectPropValue(csProdSheetUri,_T("BOF_Type")).ToLong();//熔化炉虽然定义是04，但还是按01进行逻辑
		switch(nBOFType)
		{
			case BOF50t: nBOFFlag = nOldBOF;
				break;
...
L3LONG CPlan_Mag::TappingPlanGen(L3RECORDSET rsCastingPlan)
{
	...
	while(!rsCastPlan.IsEOF())
	{
		...
		CString csLFID =  rsCastPlan.GetFieldByName(_T("LFID")).ToCString();
		CString csRHID =  rsCastPlan.GetFieldByName(_T("RHID")).ToCString();
		CString csAODID =  rsCastPlan.GetFieldByName(_T("AODID")).ToCString();
		long lTemp = GetHeatMaxProcessNo(nUnitType,csBOFID,csAODID) + 1;		//循环次数为i + 1
		...
		CString csTempBOFID = csBOFID.Mid(1,1);
        CString csHeatID ;
		if(csTempBOFID == "2")
		{
			csHeatID =_T("2") + csBOFID.Mid(2,1) + cstrCurrentDate.Mid(2,2) + csTempHeatID;
		}
		else if(csTempBOFID == "5")
		{
		    csHeatID =_T("5") + csBOFID.Mid(2,1) + cstrCurrentDate.Mid(2,2) + csTempHeatID;
		}
		...
		//更改计划订单状态
		CString cstemp1 = rsCastPlan.GetFieldByName(_T("Plan_Ord_ID")).ToCString();
...
L3LONG CPlan_Mag::CalBOFIDAndReGenTappingAct(L3SHORT nNewBOFFlag)
{
	...
	while(!rsUnExecTappingPlan.IsEOF())
	{
		//得到预定炉号(URI)
		CL3Variant valTemp = rsUnExecTappingPlan.GetFieldByName(_T("PlanID"));
		CL3Variant valAodTemp = rsUnExecTappingPlan.GetFieldByName(_T("AODID"));
		if(!valTemp.IsValid() || valTemp.IsError() || valTemp.IsNull())
			return -26;
		CString csPlanID = valTemp.ToCString();
		CString	csPlanObjUri = PLAN_TAPPING_CLASS_URI + _T("\\") + csPlanID ;
		...
		CString csAODID = arrayBOF[0].AODID;
        ...
        rsTappingPlanActual.SetFieldByName(_T("AODID"),rsUnExecTappingPlan.GetFieldByName(_T("AODID")));
		//得到熔炼号
		L3SHORT nUnitType = UNIT_AREA_BOF;
		long lTemp = GetHeatMaxProcessNo(nUnitType, csBOFID,csAODID) + 1;		//循环次数为i + 1
		CString csTempHeatID;
		...
		valAodTemp = GetObjectPropValue(csPlanObjUri,_T("AODID"));
		if(!valTemp.IsValid() || valTemp.IsError() || valTemp.IsNull())
		{
			delete arrayBOF;
			return -31;
		...
		CString csLFID = valTemp.ToCString();
		CString csAodID = valAodTemp.ToCString();
		if(csLFID!=_T("0"))
		{
			nUnitType = UNIT_AREA_LF;
			lTemp = GetHeatMaxProcessNo(nUnitType, csLFID,csAodID) + 1;		//循环次数为i + 1
			CString csTempTreatNo;
			csTempTreatNo.Format(_T("%05d"), lTemp);
			csLFTreatNo = csLFID.Mid(2,1) + cstrCurrentDate.Mid(2,2) + csTempTreatNo;
		}
		...
		{
			nUnitType = UNIT_AREA_RH;
			lTemp = GetHeatMaxProcessNo(nUnitType, csRHID,csAodID) + 1;		//循环次数为i + 1
			CString csTempTreatNo;
			csTempTreatNo.Format(_T("%05d"), lTemp);
			csRHTreatNo = csRHID.Mid(2,1) + cstrCurrentDate.Mid(2,2) + csTempTreatNo;
		}
		...
		nUnitType = UNIT_AREA_CASTER;
		lTemp = GetHeatMaxProcessNo(nUnitType, csCasterID,csAodID) + 1;		//循环次数为i + 1
		CString csTempTreatNo;
		csTempTreatNo.Format(_T("%05d"), lTemp);
		CString csCasterTreatNo = csCasterID.Mid(2,1) + cstrCurrentDate.Mid(2,2) + csTempTreatNo;
		rsTappingPlanActual.SetFieldByName(_T("Caster_TreatNo"), CL3Variant(csCasterTreatNo));
...
L3RECORDSET CPlan_Mag::GetCurDayTappingPlan(L3STRING strBofID, L3STRING strCasterID)
{
	...
	*///Modify bend
	...
	if ((csBOFID == _T("")) && (csCasterID ==_T("")))
	{
		...
		//csSQL.Format(cstrQuerySql,PLAN_TAPPING_CLASS_URI,PLAN_TAPPING_ACT_CLASS_URI,TAPPING_PLAN_STATUS_EXCUTED);
	...
	else
	{
			if(csBOFID == _T("") && csCasterID !=_T(""))
			{
				...
				CString csSQL;
				csSQL.Format(cstrQuerySql,PLAN_TAPPING_CLASS_URI,PLAN_TAPPING_ACT_CLASS_URI,PLAN_ORDER_CLASS_URI,TAPPING_PLAN_STATUS_EXCUTED,csCasterID);
			...
			else
			{
				if(csBOFID != _T("") && csCasterID ==_T(""))
				{
					...
					CString csSQL;
					csSQL.Format(cstrQuerySql,PLAN_TAPPING_CLASS_URI,PLAN_TAPPING_ACT_CLASS_URI,PLAN_ORDER_CLASS_URI,TAPPING_PLAN_STATUS_EXCUTED,csBOFID);
				...
				else
				{
					...
					CString csSQL;
					csSQL.Format(cstrQuerySql,PLAN_TAPPING_CLASS_URI,PLAN_TAPPING_ACT_CLASS_URI,PLAN_ORDER_CLASS_URI,TAPPING_PLAN_STATUS_EXCUTED,csBOFID,csCasterID);
...
L3LONG CPlan_Mag::TappingPlanChangeTimeTable(L3RECORDSET rsData)
{
	...
	CString cstrQueryL3Plan  = _T(" Select * from %s  where CasterID ='%s'      ");
			...
			cstrQueryL3Plan += _T(" and   (status=11 or status=12)                     ");//正在执行的计划 20200317
	...
	{
		//根据上一炉浇铸结束时间设定本炉开浇时刻 
        //开浇时间的计算===上一炉停浇时间+回转台操作时间（统一默认为3分钟）
...
L3LONG CPlan_Mag::CalcUnitStdProcessTime(CString csPreHeatID)
{
	...
	valTemp = GetObjectPropValue(csPlanUri,_T("Refine_Type"));//工艺路径标志
	if(!valTemp.IsValid() || valTemp.IsError() || valTemp.IsNull())
		return -1;
	...
	dtAim_IronPrepareTime = BOF_PREPARED_TIME/1440;
	dtAim_CastWaitedTime  = CASTING_WAITED_TIME/1440;
	double LF_PREPARED_TIME = 5;	
	dtAim_LFWaitTime =  LF_PREPARED_TIME/1440;
	double RH_PREPARED_TIME = 5;
	dtAim_RHWaitTime = RH_PREPARED_TIME/1440;
...
/// </Method>
 L3LONG CPlan_Mag::GetHeatMaxProcessNo(L3SHORT nUnitType, CString csUnitID,CString csAodID)
{
	...
	if (bQueryBOF)
	{
		//if(csUnitID == "S26" && csAodID ==""   )
			if(csUnitID == "S51" && csAodID ==""  )
		{
			//组成查询语句
		cstrQuery = _T("Select Max(");
		cstrQuery += cstrProcessNo;
		...
		cstrQuery += _T("  = '%s' ");
		cstrQuery += _T(" and ");
		//cstrQuery += _T(" aodid = '%s' ");
	    cstrQuery +=  _T(" aodid is null ");
		cstrQuery += _T(" and substr(");
		cstrQuery += cstrProcessNo;
		cstrQuery += _T(",1,1) = '%s' ");
		//加上年号解决跨年清零问题
	...
	if(csUnitID =="S51")
	{
	  csSQL.Format(cstrQuery,PLAN_TAPPING_ACT_CLASS_URI,csUnitID,csUnitID.Mid(1,1));
	}
	else
	{
...
L3BOOL CPlan_Mag::UpdateAODHeatid()
{
	CString strSQL;
...
L3LONG CPlan_Mag::TappingPlanGen1(L3RECORDSET rsCastingPlan)
{
	CL3RecordSetWrap rsCastPlan =rsCastingPlan;
    if( rsCastPlan.IsNull() || rsCastPlan.GetRowCount() < 1)
		return -1;
	...
	while(!rsCastPlan.IsEOF())
	{
		...
		rsTappingPlan.SetFieldByName(_T("RHID"),rsCastPlan.GetFieldByName(_T("RHID")));
		rsTappingPlan.SetFieldByName(_T("BOFID"),rsCastPlan.GetFieldByName(_T("BOFID")));
		rsTappingPlan.SetFieldByName(_T("AODID"),rsCastPlan.GetFieldByName(_T("AODID")));
		...
		rsTappingPlanActual.SetFieldByName(_T("RHID"),rsCastPlan.GetFieldByName(_T("RHID")));
		rsTappingPlanActual.SetFieldByName(_T("BOFID"),rsCastPlan.GetFieldByName(_T("BOFID")));
		rsTappingPlanActual.SetFieldByName(_T("AODID"),rsCastPlan.GetFieldByName(_T("AODID")));
	...
	//CString csSQL2=_T("select max(heatid) from cplan_tapping_act  a where bofid='S25' and to_char(CreateDate,'yyyy') = to_char(sysdate,'yyyy')");
		CString csSQL2=_T("select max(heatid) from cplan_tapping_act  a where HEATID LIKE  '6%' and to_char(CreateDate,'yyyy') = to_char(sysdate,'yyyy')");
...
L3LONG CPlan_Mag::NewCastingPlanGen1(L3RECORDSET rsPlan,L3STRING strSteelGradeIndex)
{
	...
	/*	if(rsSteelGradeIndex.GetFieldByName(_T("SteelGrade")).ToCString()!=csSteelGradeOfOrd)
		...
		CString csProdSheetUri = QA_PROD_SHEET_CLASS_URI + _T("\\") + csSteelGradeIndex;
		L3LONG	nCasterType = GetObjectPropValue(csProdSheetUri,_T("Caster_Type")).ToLong();	
		L3LONG	nAodType = GetObjectPropValue(csProdSheetUri,_T("AOD_Type")).ToLong();
		L3LONG  nCasterUnitID = _ttol(csCasterID.Mid(2,1));
		switch(nCasterType)
		{
			case NoLFCaster:
		...
		L3LONG nSteelLadleCount = CalculateHeatCount1(nBOFFlag,dSteelWeight);
		//生成浇钢计划
		L3LONG nBackValue = GenCastingPlanOfSpecifyCasterID1(csCasterID,
															csMaterialCode,
			                                                L3RECORDSET(rsPlanOrd),
															nSteelLadleCount,
															csSteelGradeIndex,
...
L3LONG CPlan_Mag::GenCastingPlanOfSpecifyCasterID1(CString csCasterID,CString csMaterialCode,L3RECORDSET rsPlanOrder,L3LONG nHeatCount,CString csSteelGradeIndex,L3DOUBLE fWeight,L3SHORT nBOFFlag,L3LONG nAodType)
{
	CL3RecordSetWrap rsPlanOrd =rsPlanOrder;
	L3DOUBLE fTotalWeight = fWeight*1000;
	...
	for(int i=0;i<nHeatCount;i++)
	{
	   ...
	   if(nBOFFlag == nNewBOF)
		...
		//计算计划开浇时间
		L3DATETIME dtCastingTime;
		if(CalculateCastingTime1(csCasterID,csSteelGradeIndex,dtCastingTime) < 0)
		{
			if(bUseTrans)
				Rollback(nTrans);
			return -97;
...
L3LONG CPlan_Mag::CalculateHeatCount1(L3SHORT nBOFFlag,L3DOUBLE dWeight)
{
	L3LONG nWeight = int((dWeight*1000));
	L3LONG nSteelLadleCount = 0;
	L3LONG nBalanceWeight =0;
...
L3LONG CPlan_Mag::CalculateCastingTime1(CString csCasterID,CString csSteelGradeIndex,L3DATETIME &dtCastingTime)
{
	L3DATETIME dtPreCastingTime;
	COleDateTime timeNow = COleDateTime::GetCurrentTime();
	CString csTemp = _T(" SELECT Max(Aim_Time_CastingStart)  FROM %s  where  CasterID = '%s' ");	
