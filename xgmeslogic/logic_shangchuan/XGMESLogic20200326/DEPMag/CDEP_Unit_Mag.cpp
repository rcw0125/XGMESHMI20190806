...
// 生产，产生新的钢水对象
	...
	CString csMaterialID = rsParam.GetFieldByName(_T("HeatID"));
	...
	//rsSteel.MoveFirst();
	//rsSteel.SetFieldByName(_T("Area"),CL3Variant(_T("S25P01")));//没用
	//CString strMaterialID;
	//
	//CString csMaterialID = rsParam.GetFieldByName(_T("MaterialID"));
	//strMaterialID = GetPropValue(_T("HeatID"));
	//
	CString csTemp = _T(" select GUID from MaterialPosition where Position ='S91' and MaterialID = '%s'");
		CString csSQL;
		...
		if (!SetObjectPropValue(_T("MES\\MaterialData\\MaterialPosition\\")+csGuid,_T("Position"),CL3Variant(_T("S25P01"))))
   {
		return FALSE; 
   }
	/*		if (!SetObjectPropValue(_T("MES\\MaterialData\\MaterialPosition\\")+csGuid,_T("MaterialID"),CL3Variant(strMaterialID)))
   {
		return FALSE; 
   }
			if (!SetObjectPropValue(_T("MES\\MaterialData\\MaterialPosition\\")+csGuid,_T("MaterialType"),CL3Variant(_T("CSteel_Data"))))
   {
		return FALSE; 
   }*/
			//
