...
{
	CString csTemp = _T(" select PlanID,CASTER_STATUS from CPlan_Tapping where  HeatID = '%s' ");
	CString csSQL;
	...
	if(!Query(csSQL,rs) || rs.IsNull() || rs.GetRowCount() < 1)
		return NULL;
	 COleDateTime curTime = COleDateTime::GetCurrentTime();
	 CString csCurDate = curTime.Format(_T("%Y-%m-%d"));
    CString csCasterStatus = rs.GetFieldByName (_T("Caster_Status")).ToString ();//铸机状态 
	 /////添加查询该炉次的生产日期 begin
	CL3RecordSetWrap rs1;
	if(csCasterStatus != "01")
	{
    CString csTemp1 = _T(" select PRODUCTIONDATE from CCCM_BASE_DATA where  HeatID = '%s' ");
	CString csSQL1;
	csSQL.Format(csTemp1,strHeatID);
	
	if(!Query(csSQL,rs1) || rs1.IsNull() || rs1.GetRowCount() < 1)
		return NULL;
	 ////添加查询该炉次的生产日期 end
	}
...
#pragma region "全回炉"
		...
		while(!rs.IsEOF())
		{
			...
			CString csTempSQL4  = _T(" select a.HeatID,'1' as Flag,a.PlanID,b.pk_poid,a.Plan_Ord_ID as jhddh,a.MATERIALCODE_ID as wlbmid,f.MaterialCode as invcode,a.HeatID as pch,case when substr(a.heatid,1,1)='2'  then  '1001NC1000000000037T' else '1001NC100000003C8ORS' end as scbmid,case when a.CASTERID ='S61'  then '1001NC100000000015R4' when a.CASTERID ='S62' then 			");
			csTempSQL4 += _T(" '1001NC100000000015R6' when a.CASTERID ='S63'then '1001NC100000000015R7' when a.CASTERID ='S64' then '1001NC100000000015R8' when a.CASTERID ='S65' then '1001NC100000000015R9' when a.CASTERID ='S67' then '1001NC100000002DPNDB' when a.CASTERID ='S66' and substr(a.HeatID,1,1)='2' then '1001NC100000001JTY8G' when a.CASTERID ='S66' and substr(a.HeatID,1,1)='6' then '1001NC100000002DDER1' when a.CASTERID ='S66' and substr(a.HeatID,1,1)='5' then '1001NC100000002DDER1' else a.CASTERID end as gzzxid,f.Bloom_Count as fjhsl,'jlda0000000000000012'as jldwid, 		");
...
#pragma region "非全回炉"
		...
		while(!rs.IsEOF())
		{
			...
			CString csTempSQL4  = _T(" select a.HeatID,'1' as Flag,a.PlanID,c.TreatNo as scddh,b.pk_poid,a.Plan_Ord_ID as jhddh,a.MATERIALCODE_ID as wlbmid,f.MaterialCode as invcode,														 ");
            ...
            CString csTEMP1;
            CString csTEMP2;
			csTEMP = rs1.GetFieldByName (_T("PRODUCTIONDATE")).ToString ();
            csTEMP1 = csTEMP.Mid (6,1);
			
            if (csTEMP1 == "-")
			{
			  //csTEMP2 = csTEMP .Left (5) +'0'+csTEMP.Mid (5,5) +'0'+csTEMP.Right (13);
				csTEMP1 = csTEMP.Mid (8,1); 
				if (csTEMP1 == " ")
				{
				 csTEMP2 = csTEMP .Left (5) +'0'+csTEMP.Mid (5,2)+'0'+csTEMP.Mid (7,2);
				 }
				else
				{
				csTEMP2 = csTEMP .Left (5) +'0'+csTEMP.Mid (5,4);
				}
				if ( csTEMP.Mid (10,1)==":")
				{
				csTEMP2 = csTEMP2 +' '+'0'+csTEMP.Right (7);
				 }
				else
                csTEMP2 = csTEMP2 +' '+ csTEMP.Right (8);
			 }

            csSJKGRQ = rsResult4.GetFieldByName (_T("sjkgrq")).ToString ();
			if (csSJKGRQ == "1899-12-30")
			{
			 rsResult4 .SetFieldByName(_T("sjkgrq"),CL3Variant(csTEMP2.Left (10)));
			 rsResult4 .SetFieldByName(_T("sjkssj"),CL3Variant(csTEMP2.Right(8)));
...
#pragma region "全回炉处理"
		...
		CString csTempSQL  = _T(" select a.HeatID,'1' as Flag,a.HeatID as pch,a.HeatID as ObjectID,a.MATERIALCODE_ID as wlbmid,									");
		...
		//转炉铁水
		//铁水
		//csTempSQL  = _T(" select NULL as pch,a.HeatID as ObjectID,'CHDA0000000000009282' as wlbmid,c.IRONWEIGHT,4 as ljcksl,																");
		csTempSQL  = _T(" select NULL as pch,a.HeatID as ObjectID,'CHDA0000000000009282' as wlbmid,round(c.IRONWEIGHT,4) as ljcksl,																");
		csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,													");
	...
	while(!rsMaterials.IsEOF())
	{
		...
		else
		{
			...
			// 0001AA1000000009UZTT 精炼合成渣XGYRZ-01 0001AA1000000003U1WJ 新冶金灰                               0001NC100000000FR9LG脱磷用氧化铁皮
			 ...
			 // 0001NC1000000010US04武安二级AOD用石灰             济南一级AOD用石灰0001NC1000000010US01            淄博一级AOD用石灰0001NC100000000L6A28         炼钢用轻烧白云石0001AA1000000006WAIQ             精炼用石灰0001NC1000000016MW9J  
			  else if(csMaterialCode == _T("0001NC1000000010US04") || csMaterialCode == _T("0001NC1000000010US01")|| csMaterialCode == _T("0001NC100000000L6A28")|| csMaterialCode == _T("0001AA1000000006WAIQ") || csMaterialCode == _T("0001NC1000000016MW9J")) 
			 {
			  rsMaterials.SetFieldByName(_T("ckckid"),CL3Variant(_T("1001NC100000001J1L5C")));//炼钢厂低位料二仓
			 }
...
#pragma region "全回炉处理"
		...
		CString csTempSQL  = _T(" select a.HeatID,'1' as Flag,a.HeatID as pch,a.HeatID as ObjectID,b.MATERIALCODE_ID as wlbmid,									");
		csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as zdrid,to_char(a.CreateDate, 'yyyy-mm-dd')  as zdrq,								");
		csTempSQL += _T(" b.ProduceStd as freeitemvalue1,b.Tech_Request as freeitemvalue2 from CPlan_Tapping a,CPlan_Order b,Cmelt_Addition_Data c		"); 
		csTempSQL += _T(" where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and c.HeatID = '%s'														");  
		CString csSQL;
		csSQL.Format(csTempSQL,csHeatID);
		CL3RecordSetWrap rsResultMain;
		...
		csTempSQL += _T(" select c.Batch_ID as pch,a.HeatID as ObjectID,(select distinct NC_CODE from CBASE_ALL_CODE where CODE =c.ELEMENT and Code_Group ='ALLOY_CODE') as wlbmid,c.WEIGHT/1000 as ljcksl,  		");
		csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,																		 		");
		csTempSQL += _T(" '1001NC100000002DDER1' as gzzxid,To_Char(d.App_Time,'yyyy-mm-dd') as flrq,'1006AA100000000E8U7E' as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_Addition_Data c, 									 		");
		csTempSQL += _T(" CDisp_SteelTurn_App d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'	and c.Type=5  and c.Element <> '67103' and d.ALL_TURN_FLAG = 1  "); 
		csTempSQL += _T(" )group by pch,ObjectID,wlbmid,jldwid,kgyid,gzzxid,flrq,ckckid	                                               																                "); 
		csSQL.Format(csTempSQL,csHeatID);
		CL3RecordSetWrap rsResultBOF1;
		...
		csTempSQL += _T(" a.HeatID as ObjectID,(select distinct NC_CODE from CBASE_ALL_CODE where CODE =c.ELEMENT and Code_Group ='BULK_CODE') as wlbmid,c.WEIGHT/1000 as ljcksl,									");
		csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,																				");
		csTempSQL += _T("  '1001NC100000002DDER1' as gzzxid,To_Char(d.App_Time,'yyyy-mm-dd') as flrq,  'CKDA0000000000000007'   as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_Addition_Data c, 											");
		csTempSQL += _T(" CDisp_SteelTurn_App d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'	and c.Type < 5 and c.Element <> '67103' and d.ALL_TURN_FLAG = 1 )	"); 	
		csTempSQL += _T(" group by pch,ObjectID,wlbmid,jldwid,kgyid,gzzxid,flrq,ckckid																																"); 
		csSQL.Format(csTempSQL,csHeatID);
		CL3RecordSetWrap rsResultBOF2;
		...
		csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,													");
		//csTempSQL += _T("  a.BOFID ='S22' then '1001NC100000000015QN' when a.BOFID ='S23' then '1001NC100000000015QS' when a.BOFID ='S24' then '1001NC100000000015QU' else a.BOFID end	");
		csTempSQL += _T("  '1001NC100000002DDER1' as gzzxid,To_Char(d.App_Time,'yyyy-mm-dd') as flrq,'CKDA0000000000000026' as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_FEED_DATA c, 					");
		csTempSQL += _T(" CDisp_SteelTurn_App d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s' and d.ALL_TURN_FLAG = 1				"); 
		csSQL.Format(csTempSQL,csHeatID);
		CL3RecordSetWrap rsResultBOF3;
		if(!Query(csSQL,rsResultBOF3))
		...
		///////////////////20160319 屏蔽生铁废钢
		//生铁
		csTempSQL  = _T(" select NULL as pch,a.HeatID as ObjectID,'0001NC10000000003MYN' as wlbmid,round(c.PIGIRONWEIGHT,4)  as ljcksl,																");
		csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,													");
		//csTempSQL += _T("  a.BOFID ='S22' then '1001NC100000000015QN' when a.BOFID ='S23' then '1001NC100000000015QS' when a.BOFID ='S24' then '1001NC100000000015QU' else a.BOFID end		");
		csTempSQL += _T(" '1001NC100000002DDER1' as gzzxid,To_Char(d.App_Time,'yyyy-mm-dd') as flrq,'CKDA0000000000000009' as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_FEED_DATA c, 						");
		csTempSQL += _T(" CDisp_SteelTurn_App d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s' and d.ALL_TURN_FLAG = 1 					"); 
		csSQL.Format(csTempSQL,csHeatID);
		CL3RecordSetWrap rsResultBOF4;
		if(!Query(csSQL,rsResultBOF4))
		...
		//废钢
		csTempSQL  = _T(" select NULL as pch,a.HeatID as ObjectID,'0001NC10000000003MYM' as wlbmid,round(c.SCRAPWEIGHT,4) as ljcksl,																	");
		csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,														");
		//csTempSQL += _T("  a.BOFID ='S22' then '1001NC100000000015QN' when a.BOFID ='S23' then '1001NC100000000015QS' when a.BOFID ='S24' then '1001NC100000000015QU' else a.BOFID end		");
		csTempSQL += _T("  '1001NC100000002DDER1' as gzzxid,To_Char(d.App_Time,'yyyy-mm-dd') as flrq,'CKDA0000000000000009' as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_FEED_DATA c, 						");
		csTempSQL += _T(" CDisp_SteelTurn_App d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'  and d.ALL_TURN_FLAG = 1 					"); 
...
#pragma region "拨出或者不拨坯处理"
			...
			csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,																			");
			//csTempSQL += _T("  a.BOFID ='S22' then '1001NC100000000015QN' when a.BOFID ='S23' then '1001NC100000000015QS' when a.BOFID ='S24' then '1001NC100000000015QU' else a.BOFID end							");
			csTempSQL += _T(" '1001NC100000002DDER1' as gzzxid,To_Char(d.CastingStartTime,'yyyy-mm-dd') as flrq, '1001NC100000001J1L5B'   as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_Addition_Data c, 								");
			csTempSQL += _T(" CCCM_Process_Data d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'		and c.Element <> '67103'						");
			csTempSQL += _T(" )group by pch,ObjectID,wlbmid,jldwid,kgyid,gzzxid,flrq,ckckid																															"); 
			...
			csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,													");
			//csTempSQL += _T("  a.BOFID ='S22' then '1001NC100000000015QN' when a.BOFID ='S23' then '1001NC100000000015QS' when a.BOFID ='S24' then '1001NC100000000015QU' else a.BOFID end		");
			csTempSQL += _T("  '1001NC100000002DDER1' as gzzxid,To_Char(d.CastingStartTime,'yyyy-mm-dd') as flrq, '1001NC100000001J1L5C' as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_Addition_Data c, 			");
			csTempSQL += _T(" CCCM_Process_Data d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'	and c.Type < 5 and c.Element <> '67103')	"); 	
			csTempSQL += _T(" group by pch,ObjectID,wlbmid,jldwid,kgyid,gzzxid,flrq,ckckid																										"); 
			csSQL.Format(csTempSQL,fBanlance,csHeatID);
			...
			csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,													");
			//csTempSQL += _T("  a.BOFID ='S22' then '1001NC100000000015QN' when a.BOFID ='S23' then '1001NC100000000015QS' when a.BOFID ='S24' then '1001NC100000000015QU' else a.BOFID end		");
			csTempSQL += _T("  '1001NC100000002DDER1' as gzzxid,To_Char(d.CastingStartTime,'yyyy-mm-dd') as flrq,'1001NC100000001J1L6O' as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_FEED_DATA c, 				");
			csTempSQL += _T(" CCCM_Process_Data d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'												"); 
			csSQL.Format(csTempSQL,fBanlance,csHeatID);
			...
			csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,													");
			//csTempSQL += _T("  a.BOFID ='S22' then '1001NC100000000015QN' when a.BOFID ='S23' then '1001NC100000000015QS' when a.BOFID ='S24' then '1001NC100000000015QU' else a.BOFID end		");
			csTempSQL += _T("  '1001NC100000002DDER1' as gzzxid,To_Char(d.CastingStartTime,'yyyy-mm-dd') as flrq,'1001NC100000001J1L5D' as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_FEED_DATA c, 				");
			csTempSQL += _T(" CCCM_Process_Data d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'												"); 
			csSQL.Format(csTempSQL,fBanlance,csHeatID);
...
#pragma endregion
		...
		{
#pragma region "拨入钢坯"
			////add by YUAN 2017-11-24  对拨入炉号的仓库（不锈钢合金库和低位料仓库） 进行区分AddHeatID1
			//CL3RecordSetWrap rsPlanTappingAdd1;
			//CString csSql;CString csAddCasterID1;CString csAddLFID1 ;CString csAddBofID1;
			//csSql.Format(_T("select BOFID,CASTERID,LFID from %s where HEATID='%s'"),PLAN_TAPPING_CLASS_URI,strAddHeatID1);
			//if(Query(csSql,rsPlanTappingAdd1)&&rsPlanTappingAdd1.GetRowCount()>0)
			//{
			//	CString csTempSQL;
			//	CString csSQL;
			//	rsPlanTappingAdd1.MoveFirst();
			//	csAddBofID1 = rsPlanTappingAdd1.GetFieldByName (_T("BOFID")).ToString ();//转炉号
			//	csAddCasterID1=rsPlanTappingAdd1.GetFieldByName(_T("CASTERID")).ToString();	//铸机号
			//	csAddLFID1=rsPlanTappingAdd1.GetFieldByName(_T("LFID")).ToString(); //精炼号
			//}
			////end
			////add by YUAN 2017-11-24  对拨入炉号的仓库（不锈钢合金库和低位料仓库） 进行区分AddHeatID2
			//CL3RecordSetWrap rsPlanTappingAdd2;
			//CString csSql;CString csAddCasterID2;CString csAddLFID2 ;CString csAddBofID2;
			//csSql.Format(_T("select BOFID,CASTERID,LFID from %s where HEATID='%s'"),PLAN_TAPPING_CLASS_URI,strAddHeatID2);
			//if(Query(csSql,rsPlanTappingAdd2)&&rsPlanTappingAdd2.GetRowCount()>0)
			//{
			//	CString csTempSQL;
			//	CString csSQL;
			//	rsPlanTappingAdd2.MoveFirst();
			//	csAddBofID2 = rsPlanTappingAdd2.GetFieldByName (_T("BOFID")).ToString ();//转炉号
			//	csAddCasterID2=rsPlanTappingAdd2.GetFieldByName(_T("CASTERID")).ToString();	//铸机号
			//	csAddLFID2=rsPlanTappingAdd2.GetFieldByName(_T("LFID")).ToString(); //精炼号
			//}
			////end
			//根据拨入炉号获取拨入炉号的计划钢坯支数
			L3DOUBLE nAddPlanBloomCount1 = 0;
...
#pragma region "熔化炉合金本炉总的投料"
			...
			if(csCasterID==_T("S66"))
			{
				if (strAddHeatID1.Mid (0,1)==_T("5") || strAddHeatID2.Mid (0,1)==_T("5"))
				{
				csTempSQL  = _T(" select pch,ObjectID,wlbmid,round(sum(ljcksl) * '%f',6)  as ljcksl,jldwid,kgyid,gzzxid,flrq,ckckid from(																			    ");
				csTempSQL += _T(" select c.Batch_ID as pch,\'");
				csTempSQL += csHeatID + _T("\'  as ObjectID,(select distinct NC_CODE from CBASE_ALL_CODE where CODE =c.ELEMENT and Code_Group ='ALLOY_CODE') as wlbmid,c.WEIGHT/1000 as ljcksl,		                    ");
				csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,\'");
				csTempSQL += csNCCenter;
				csTempSQL += _T("\' as gzzxid,To_Char(d.CastingStartTime,'yyyy-mm-dd') as flrq,  '1001NC100000001J1L5B'  as ckckid from CPlan_Tapping a,CPlan_Order b,CMELT_Addition_Data c, 								");
				csTempSQL += _T(" CCCM_Process_Data d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'	and c.Type=5	and c.Element <> '67103'						");
				csTempSQL += _T(" )group by pch,ObjectID,wlbmid,jldwid,kgyid,gzzxid,flrq,ckckid																															"); 
				}
				else
...
#pragma region "本炉熔化炉辅料"
			...
			if(csCasterID==_T("S66"))
			{
				...
				csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,\'");
				csTempSQL += csNCCenter;
				csTempSQL += _T("\' as gzzxid,To_Char(d.CastingStartTime,'yyyy-mm-dd') as flrq,  '1001NC100000001J1L5C'    as ckckid from CPlan_Tapping a,CPlan_Order b,Cmelt_Addition_Data c, 			");
				csTempSQL += _T(" CCCM_Process_Data d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'	and c.Type < 5 and c.Element <> '67103')	"); 	
				csTempSQL += _T(" group by pch,ObjectID,wlbmid,jldwid,kgyid,gzzxid,flrq,ckckid																										"); 
				}
				else
...
#pragma region "熔化炉本炉铁水"
			...
			if(csBofID ==_T("S51"))
			{
				if (strAddHeatID1.Mid (0,1)==_T("5") || strAddHeatID2.Mid (0,1)==_T("5"))
				{
					...
					csTempSQL += _T(" 'jlda0000000000000012'as jldwid,a.Planner as kgyid,\'");
					csTempSQL += csNCCenter;
					csTempSQL += _T("\'  as gzzxid,To_Char(d.CastingStartTime,'yyyy-mm-dd') as flrq,'1001NC100000001J1L6O' as ckckid from CPlan_Tapping a,CPlan_Order b,CMELT_FEED_DATA c, 				");
					csTempSQL += _T(" CCCM_Process_Data d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'												");
				}
				else
				{
...
#pragma region "LF本炉合金"
			...
			//按照拨入炉号查询但是将拨入炉号，投料中心全部换为当前炉,LF投料中心不用换,ckckid根据拨入炉号判断发料仓库为炼钢合金二库还是炼钢合金库
			if(csLFID==_T("S44"))
			{
				csTempSQL  = _T(" select pch,ObjectID,wlbmid,round(sum(ljcksl) * '%f' ,6)as ljcksl,jldwid,kgyid,gzzxid,flrq,ckckid from(																	    ");
				csTempSQL += _T(" select c.Batch_ID as pch,\'");
				...
				csTempSQL += _T(" as wlbmid,c.WEIGHT/1000 as ljcksl,'jlda0000000000000012'as jldwid,a.Planner as kgyid,case when substr(c.TreatNo,1,1) ='1' then '1001NC100000000015QW' when		            ");
				csTempSQL += _T("  substr(c.TreatNo,1,1) ='2' then '1001NC100000000015R0' when substr(c.TreatNo,1,1) ='3' then '1001NC100000000015R2' when substr(c.TreatNo,1,1) ='4' then '1001NC100000002DDER1' when substr(c.TreatNo,1,1) ='5' then '1001NC100000002DPNCG' else a.LFID end as gzzxid,	  ");//modify by hyh 2012-06-08 新增4#LF
				csTempSQL += _T("  To_Char(d.CastingStartTime,'yyyy-mm-dd') as flrq, case when a.bofid ='S51'then '1001NC100000001J1L5B' when a.bofid <>'S51' then '1006AA100000000E8U7E' end  as ckckid from CPlan_Tapping a,CPlan_Order b,CLF_Addition_Data c, 	                                ");
				csTempSQL += _T(" CCCM_Process_Data d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'	and c.Element <> '67103'						        ");
				csTempSQL += _T(" and c.TreatNo is not null)group by pch,ObjectID,wlbmid,jldwid,kgyid,gzzxid,flrq,ckckid																		                ");
			}
			else
...
#pragma region "LF本炉喂线"
			...
			if(csLFID==_T("S44"))
			{
				...
				csTempSQL += _T(" as wlbmid,c.Amount/1000 as ljcksl,'jlda0000000000000012'as jldwid,a.Planner as kgyid,case when substr(c.TreatNo,1,1) ='1' then '1001NC100000000015QW' when ");
				csTempSQL += _T(" substr(c.TreatNo,1,1) ='2' then '1001NC100000000015R0' when substr(c.TreatNo,1,1) ='3' then '1001NC100000000015R2'  when substr(c.TreatNo,1,1) ='4' then '1001NC100000002DDER1' when substr(c.TreatNo,1,1) ='5' then '1001NC100000002DPNCG' else a.LFID end as gzzxid,		         ");//modify by hyh 2012-06-08 新增4#LF
				csTempSQL += _T(" To_Char(d.CastingStartTime,'yyyy-mm-dd') as flrq, case when a.bofid ='S51'then '1001NC100000001J1L5B' when a.bofid <>'S51' then '1006AA100000000E8U7E' end    as ckckid from CPlan_Tapping a,CPlan_Order b,CLF_WIRE_DATA c, 				     ");
				csTempSQL += _T(" CCCM_Process_Data d where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and a.HeatID =d.HeatID and c.HeatID = '%s'									     ");
				csTempSQL += _T(" and c.TreatNo is not null)group by pch,ObjectID,wlbmid,jldwid,kgyid,gzzxid,flrq,ckckid																	 ");
			}
			else
	...
	while(!rsMaterials.IsEOF())
	{
		...
		else
		{
			...
			// 0001AA1000000009UZTT 精炼合成渣XGYRZ-01 0001AA1000000003U1WJ 新冶金灰                               0001NC100000000FR9LG脱磷用氧化铁皮
			 ...
			 // 0001NC1000000010US04武安二级AOD用石灰             济南一级AOD用石灰0001NC1000000010US01            淄博一级AOD用石灰0001NC100000000L6A28         炼钢用轻烧白云石0001AA1000000006WAIQ             //精炼用石灰0001NC1000000016MW9J
			  //else if(csMaterialCode == _T("0001NC1000000010US04") || csMaterialCode == _T("0001NC1000000010US01")|| csMaterialCode == _T("0001NC100000000L6A28")|| csMaterialCode == _T("0001AA1000000006WAIQ")|| csMaterialCode == _T("0001NC1000000016MW9J")) 
			  else if(csMaterialCode == _T("0001NC1000000010US04") || csMaterialCode == _T("0001NC1000000010US01")|| csMaterialCode == _T("0001NC100000000L6A28")) 
			 {
			  rsMaterials.SetFieldByName(_T("ckckid"),CL3Variant(_T("1001NC100000001J1L5C")));//炼钢厂低位料二仓
			 }
			 //CHDA0000000000009283 生铁  0001NC100000000JJH0Z 镍系不锈钢切废 0001NC100000000J2LA7钼系不锈钢切废 0001NC100000000J2LA9铜系不锈钢切废 0001NC100000000JJH1V铬系不锈钢切废     
...
#pragma region "删除垃圾数据"
	...
	//		//0001NC1000000010US04武安二级AOD用石灰             济南一级AOD用石灰0001NC1000000010US01 ---2016-09-06                                                                                                精炼用石灰0001NC1000000016MW9J
	//		else if((csMaterialCode == _T("0001AA1000000009UZTT") || csMaterialCode == _T("0001AA1000000003U1WJ")||csMaterialCode == _T("0001NC1000000010US04") || csMaterialCode == _T("0001NC1000000010US01") ||csMaterialCode == _T("0001NC1000000016MW9J")) && csCasterID==_T("S66") && csCenter==_T("1001NC100000001KDV0W"))
	//			rsMaterials.SetFieldByName(_T("ckckid"),CL3Variant(_T("1001NC100000001J1L5C")));//炼钢厂低位料二仓
...
#pragma region "非全回炉"
		...
		{		
			...
			if(nReturnWeight > 0)
			{
				...
				csTempSQL  = _T(" select a.TreatNo,NULL as pch,c.Planner as zdrid,to_Char(sysdate,'hh24:mi:ss') as sj,to_Char(sysdate,'yyyy-mm-dd') as rq,													");   
				csTempSQL += _T(" case when a.CCMID ='S61' then '1001NC100000000015R4' when a.CCMID ='S62' then '1001NC100000000015R6' when a.CCMID ='S63' then                                             ");
				csTempSQL += _T(" '1001NC100000000015R7' when a.CCMID ='S64' then '1001NC100000000015R8' when a.CCMID ='S65' then '1001NC100000000015R9'  when a.CCMID ='S66' and substr(a.HeatID,1,1)='2' then '1001NC100000001JTY8G'  when a.CCMID ='S66' and substr(a.HeatID,1,1)='6' then '1001NC100000002DDER1' when a.CCMID ='S66' and substr(a.HeatID,1,1)='5' then '1001NC100000002DDER1' when a.CCMID ='S67' then '1001NC100000002DPNDB' end as gzzxid,a.TreatNo as ccxh,  				");//modify by hyh 2012-06-08 新增CCM7
				csTempSQL += _T(" case when a.CCMID ='S61' then '110301' when a.CCMID ='S62' then '110302' when a.CCMID ='S63' then																			");
				csTempSQL += _T(" '110303' when a.CCMID ='S64' then '110304' when a.CCMID ='S65' then '110305' when a.CCMID ='S67' then '110306'  when a.CCMID ='S66' and substr(a.HeatID,1,1)='2' then '140301' when a.CCMID ='S66'and substr(a.HeatID,1,1)='6' then '140311'when a.CCMID ='S66'and substr(a.HeatID,1,1)='5' then '140311' end as gzzxbmid,																			");//modify by hyh 2012-06-08 新增CCM7
				csTempSQL += _T(" '0001AA1000000002IQ4W' as wlbmid,'jlda0000000000000012'as jldwid,																											");  
				csTempSQL += _T(" d.ProduceStd as zyx1,d.Tech_Request as zyx2,to_Char(a.CASTINGSTARTTIME,'yyyy-mm-dd') as Ksrq,to_Char(a.CASTINGSTARTTIME,'hh24:mi:ss') as Kssj,				            ");   
				csTempSQL += _T(" to_Char(a.CASTINGSTOPTIME,'yyyy-mm-dd') as Jsrq,to_Char(a.CASTINGSTOPTIME,'hh24:mi:ss') as Jssj,a.Weight_Return as hgsl,'Y' as sflfcp,									");   
				csTempSQL += _T(" a.TreatNo||a.HeatID as ObjectID,'N' as sffsgp                                                                                                                             ");   
			...
			csTempSQL  = _T(" select a.TreatNo,a.HeatID,'1' as Flag,a.HeatID as pch,c.Planner as zdrid,to_Char(sysdate,'hh24:mi:ss') as sj,to_Char(sysdate,'yyyy-mm-dd') as rq,							");   
			csTempSQL += _T(" case when a.CCMID ='S61' then '1001NC100000000015R4' when a.CCMID ='S62' then '1001NC100000000015R6' when a.CCMID ='S63' then                                             ");
			csTempSQL += _T(" '1001NC100000000015R7' when a.CCMID ='S64' then '1001NC100000000015R8' when a.CCMID ='S65' then '1001NC100000000015R9' when a.CCMID ='S67' then '1001NC100000002DPNDB'  when a.CCMID ='S66' and substr(a.HeatID,1,1)='2' then '1001NC100000001JTY8G' when a.CCMID ='S66' and substr(a.HeatID,1,1)='6' then '1001NC100000002DDER1' when a.CCMID ='S66' and substr(a.HeatID,1,1)='5' then '1001NC100000002DDER1' end as gzzxid,a.TreatNo as ccxh,  				");//Modify by hyh 2012-06-08 新增CCM7
			csTempSQL += _T(" case when a.CCMID ='S61' then '110301' when a.CCMID ='S62' then '110302' when a.CCMID ='S63' then																			");
			csTempSQL += _T(" '110303' when a.CCMID ='S64' then '110304' when a.CCMID ='S65' then '110305' when a.CCMID ='S67' then '110306' when a.CCMID ='S66' and substr(a.HeatID,1,1)='2' then '140301' when a.CCMID ='S66'and substr(a.HeatID,1,1)='6' then '140311' when a.CCMID ='S66'and substr(a.HeatID,1,1)='5' then '140311' end as gzzxbmid,																			");//Modify by hyh 2012-06-08 新增CCM7
			//csTempSQL += _T(" d.MaterialCode_ID as wlbmid,'jlda0000000000000012'as jldwid,																												");  
			csTempSQL += _T(" c.MaterialCode_ID as wlbmid,'jlda0000000000000012'as jldwid,																												");  
			csTempSQL += _T(" d.ProduceStd as zyx1,d.Tech_Request as zyx2,to_Char(a.CASTINGSTARTTIME,'yyyy-mm-dd') as Ksrq,to_Char(a.CASTINGSTARTTIME,'hh24:mi:ss') as Kssj,				            ");   
			csTempSQL += _T(" to_Char(a.CASTINGSTOPTIME,'yyyy-mm-dd') as Jsrq,to_Char(a.CASTINGSTOPTIME,'hh24:mi:ss') as Jssj,b.CAL_WEIGHT as hgsl,b.BLOOM_COUNT as fhgsl,                              "); 
			csTempSQL += _T(" 'N' as sflfcp,                                                                                                                                                            ");             
			csTempSQL += _T(" a.TreatNo||a.HeatID as ObjectID,case when b.Pre_SteelGradeIndex = b.SteelGradeIndex or b.SteelGradeIndex ='YY000000' then 'N' else 'Y' end as sffsgp,                     ");   
...
#pragma region "全回炉"
		...
		csTempSQL += _T(" '110302' when c.CASTERID ='S63' then '110303' when c.CASTERID ='S64'  then '110304' when c.CASTERID ='S65' then '110305'  when c.CASTERID ='S67' then '110306'  when c.CASTERID ='S66' and substr(a.HeatID,1,1)='2' then '140301'  when c.CASTERID ='S66'and substr(a.HeatID,1,1)='6' then '140311'when c.CASTERID ='S66'and substr(a.HeatID,1,1)='5' then '140311' end as gzzxbmid,					");//Modify by hyh 2012-06-08 新增CCM7
...
//热送  2016-01-05
			CString csTempSQL1  = _T(" select c.HeatID,'1' as Flag,a.MaterialID as ObjectID,(select NC_Code from CBase_All_Code where Code = e.Target ) as cwarehouseid,To_Char(f.CASTINGSTARTTIME,'yyyy-mm-dd hh24:mi:ss') as taccounttime,a.Operator as coperatorid,NULL as TreatNo,"); 
			csTempSQL1 += _T(" '1001NC100000000052Z3' as ccheckstate_bid,case when b.CASTERID ='S61' then '1001NC100000000015R4' when b.CASTERID ='S62' then '1001NC100000000015R6' when b.CASTERID ='S63' then 					                                          ");
			csTempSQL1 += _T(" '1001NC100000000015R7' when b.CASTERID ='S64' then '1001NC100000000015R8' when b.CASTERID ='S65' then '1001NC100000000015R9' when b.CASTERID ='S67' then '1001NC100000002DPNDB'  when b.CASTERID ='S66' and substr(c.HeatID,1,1)='2' then '1001NC100000001JTY8G'  when b.CASTERID ='S66' and (substr(c.HeatID,1,1)='6'or substr(c.HeatID,1,1)='5') then '1001NC100000002DDER1' end as cworkcenterid,c.MATERIALCODE_ID as cinvbasid, 		    ");//add by hyh 2012-06-08 新增CCM7
			csTempSQL1 += _T(" To_Char(f.CASTINGSTARTTIME,'yyyy-mm-dd') as dbizdate,a.MaterialID as vbatchcode,(a.Right_Weight-a.WASTER_WEIGHT) as ninnum,a.RIGHT_COUNT as ninassistnum,'jlda0000000000000019'								                                  ");
			csTempSQL1 += _T(" as castunitid,d.ProduceStd as vfree1,d.Tech_Request as vfree2 from CBloom_Input_Log a,CBLOOM_DATA b,CPlan_Tapping c,CPlan_Order d,CBloom_Output_Log e,CCCM_Process_Data f where a.MaterialID = e.MaterialID and 			                      ");
			csTempSQL1 += _T(" a.Type = '9' and a.Right_Count > 0  and a.MaterialID = b.MaterialID and c.Plan_Ord_ID = d.PlanID and c.HeatID =b.HeatID and f.HeatID =b.HeatID and b.HeatID = '%s'	order by a.Logdate desc													  ");
...
#pragma region "不合格原因1"
			...
			csTempSQL3 += _T(" '1001NC100000000052Z6' as ccheckstate_bid,case when b.CASTERID ='S61' then '1001NC100000000015R4' when b.CASTERID ='S62' then '1001NC100000000015R6' when b.CASTERID ='S63' then 					");
			csTempSQL3 += _T(" '1001NC100000000015R7' when b.CASTERID ='S64' then '1001NC100000000015R8' when b.CASTERID ='S65' then '1001NC100000000015R9' when b.CASTERID ='S67' then '1001NC100000002DPNDB'  when b.CASTERID ='S66' and substr(b.heatid,1,1)='2' then '1001NC100000001JTY8G' when b.CASTERID ='S66' and (substr(c.HeatID,1,1)='6'or substr(c.HeatID,1,1)='5')  then '1001NC100000002DDER1' end as cworkcenterid,c.MATERIALCODE_ID as cinvbasid, 					");//add by hyh 2012-06-08 新增CCM7
			csTempSQL3 += _T(" To_Char(f.CASTINGSTARTTIME,'yyyy-mm-dd') as dbizdate,a.MaterialID as vbatchcode,b.Wrong_Weight1 as ninnum,b.Wrong_Count1 as ninassistnum,case when b.Wrong_Count1>0 then 'jlda0000000000000019' else '' end	");
			csTempSQL3 += _T(" as castunitid,d.ProduceStd as vfree1,d.Tech_Request as vfree2 from CBloom_Input_Log a,CBLOOM_DATA b,CPlan_Tapping c,CPlan_Order d,CCCM_Process_Data f  where a.MaterialID = b.MaterialID and 				");
			csTempSQL3 += _T(" b.Wrong_Count1 > 0 and c.Plan_Ord_ID = d.PlanID and c.HeatID =b.HeatID and f.HeatID =b.HeatID and b.HeatID = '%s'	order by a.Logdate desc		");
...
#pragma region "不合格原因2"
			...
			csTempSQL31 += _T(" '1001NC100000000052Z6' as ccheckstate_bid,case when b.CASTERID ='S61' then '1001NC100000000015R4' when b.CASTERID ='S62' then '1001NC100000000015R6' when b.CASTERID ='S63' then 					");
			csTempSQL31 += _T(" '1001NC100000000015R7' when b.CASTERID ='S64' then '1001NC100000000015R8' when b.CASTERID ='S65' then '1001NC100000000015R9'  when b.CASTERID ='S67' then '1001NC100000002DPNDB'  when b.CASTERID ='S66' and  substr(c.heatid,1,1)='2' then '1001NC100000001JTY8G' when b.CASTERID ='S66' and (substr(c.HeatID,1,1)='6'or substr(c.HeatID,1,1)='5')  then '1001NC100000002DDER1'  end as cworkcenterid,c.MATERIALCODE_ID as cinvbasid, 					");//add by hyh 2012-06-08
			csTempSQL31 += _T(" To_Char(f.CASTINGSTARTTIME,'yyyy-mm-dd') as dbizdate,a.MaterialID as vbatchcode,b.Wrong_Weight2 as ninnum,b.Wrong_Count2 as ninassistnum,case when b.Wrong_Count2>0 then 'jlda0000000000000019' else '' end	");
			csTempSQL31 += _T(" as castunitid,d.ProduceStd as vfree1,d.Tech_Request as vfree2 from CBloom_Input_Log a,CBLOOM_DATA b,CPlan_Tapping c,CPlan_Order d,CCCM_Process_Data f  where a.MaterialID = b.MaterialID and 		");
			csTempSQL31 += _T(" b.Wrong_Count2 > 0 and c.Plan_Ord_ID = d.PlanID and c.HeatID =b.HeatID and f.HeatID =b.HeatID and b.HeatID = '%s'	order by a.Logdate desc		");
			//Modify end
...
#pragma region "回炉钢"
			...
			csTempSQL5 += _T(" '1001NC100000000052Z3' as ccheckstate_bid,case when b.CASTERID ='S61' then '1001NC100000000015R4' when b.CASTERID ='S62' then '1001NC100000000015R6' when b.CASTERID ='S63' then 				");
			csTempSQL5 += _T(" '1001NC100000000015R7' when b.CASTERID ='S64' then '1001NC100000000015R8' when b.CASTERID ='S65' then '1001NC100000000015R9' when b.CASTERID ='S67' then '1001NC100000002DPNDB'  when b.CASTERID ='S66' and substr(c.heatid,1,1)='2' then '1001NC100000001JTY8G' when b.CASTERID ='S66' and (substr(c.HeatID,1,1)='6'or substr(c.HeatID,1,1)='5')  then '1001NC100000002DDER1' end as cworkcenterid,'0001AA1000000002IQ4W' as cinvbasid, 			");//add by hyh 2012-06-08
			csTempSQL5 += _T(" To_Char(e.CASTINGSTARTTIME,'yyyy-mm-dd') as dbizdate,NULL as vbatchcode,e.Weight_Return as ninnum,d.ProduceStd as vfree1,d.Tech_Request as vfree2 from CBloom_Input_Log a,CBLOOM_DATA b,CPlan_Tapping c,  ");
			csTempSQL5 += _T(" CPlan_Order d,CCCM_Process_Data e where a.MaterialID = b.MaterialID and e.Weight_Return > 0 and c.Plan_Ord_ID = d.PlanID and c.HeatID =b.HeatID and b.HeatID =e.HeatID and b.HeatID = '%s' order by a.Logdate desc		");
...
#pragma region "全回炉"
		...
		csTempSQL += _T(" case when a.CASTERID ='S61' then '1001NC100000000015R4' when a.CASTERID ='S62' then '1001NC100000000015R6' when a.CASTERID ='S63' then 													");
		csTempSQL += _T(" '1001NC100000000015R7' when a.CASTERID ='S64' then '1001NC100000000015R8' when a.CASTERID ='S65' then '1001NC100000000015R9' when a.CASTERID ='S67' then '1001NC100000002DPNDB'  when b.CASTERID ='S66' and substr(c.heatid,1,1)='2' then '1001NC100000001JTY8G' when b.CASTERID ='S66' and (substr(c.HeatID,1,1)='6'or substr(c.HeatID,1,1)='5')  then '1001NC100000002DDER1'  end as cworkcenterid,'0001AA1000000002IQ4W' as cinvbasid, 	");//add by hyh 2012-06-08
		csTempSQL += _T(" To_Char(c.App_Time,'yyyy-mm-dd') as dbizdate,NULL as vbatchcode,round(c.App_Steel_Weight,3) as ninnum from CPlan_Tapping a,																	");
		csTempSQL += _T(" CPlan_Order b,CDisp_SteelTurn_App c where a.Plan_Ord_ID = b.PlanID and a.HeatID =c.HeatID and c.HeatID = '%s'	 and c.ALL_TURN_FLAG = 1 													");
