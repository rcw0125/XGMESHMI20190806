...
{
    public partial class ccmReScrapAllFrm : DockContent, IMESForm
    {
        public ccmReScrapAllFrm()
        {
            InitializeComponent();
        }
        #region IMESForm 成员
        ...
        {
            string strsql = " select  materialid from cbloom_data   where  status >=11   ";
            strsql += " and materialid in (select heatid from cccm_base_data where productiondate > sysdate-2)";
        ...
        {        
            ...
            if (MessageBox.Show(comboBox1.Text.Trim()+"炉次确定要做甩废吗", "请确认", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                string note = "ip地址" + GetLocalIP() + ",NC上传后重新甩废炉号：" + comboBox1.Text.Trim();
